/**
 * esbuild configuration for building the crixlyai CLI for npm.
 *
 * Bundles all workspace packages (@crixlyai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @crixlyai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@crixlyai/")) {
      externals.add(name);
    }
  }
