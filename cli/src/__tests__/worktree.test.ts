...
function buildSourceConfig(): CrixlyConfig {
  return {
    $meta: {
      version: 1,
...
describe("worktree helpers", () => {
  ...
  it("resolves worktree:make target paths under the user home directory", () => {
    expect(resolveWorktreeMakeTargetPath("crixly-pr-432")).toBe(
      path.resolve(os.homedir(), "crixly-pr-432"),
    );
  });

  it("rejects worktree:make names that are not safe directory/branch names", () => {
    expect(() => resolveWorktreeMakeTargetPath("crixly/pr-432")).toThrow(
  ...
  it("rewrites loopback auth URLs to the new port only", () => {
    ...
    expect(rewriteLocalUrlPort("https://crixly.example", 3110)).toBe("https://crixly.example");
  });

  it("builds isolated config and env paths for a worktree", () => {
    const paths = resolveWorktreeLocalPaths({
      cwd: "/tmp/crixly-feature",
      homeDir: "/tmp/crixly-worktrees",
    ...
    expect(config.database.embeddedPostgresDataDir).toBe(
      path.resolve("/tmp/crixly-worktrees", "instances", "feature-worktree-support", "db"),
    );
    ...
    expect(config.storage.localDisk.baseDir).toBe(
      path.resolve("/tmp/crixly-worktrees", "instances", "feature-worktree-support", "data", "storage"),
    );
    ...
    expect(env.CRIXLY_HOME).toBe(path.resolve("/tmp/crixly-worktrees"));
    expect(env.CRIXLY_INSTANCE_ID).toBe("feature-worktree-support");
    expect(env.CRIXLY_IN_WORKTREE).toBe("true");
    expect(env.CRIXLY_WORKTREE_NAME).toBe("feature-worktree-support");
    expect(env.CRIXLY_WORKTREE_COLOR).toBe("#3abf7a");
    expect(formatShellExports(env)).toContain("export CRIXLY_INSTANCE_ID='feature-worktree-support'");
  });
  ...
  it("copies the source local_encrypted secrets key into the seeded worktree instance", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "crixly-worktree-secrets-"));
    const originalInlineMasterKey = process.env.CRIXLY_SECRETS_MASTER_KEY;
    const originalKeyFile = process.env.CRIXLY_SECRETS_MASTER_KEY_FILE;
    try {
      delete process.env.CRIXLY_SECRETS_MASTER_KEY;
      delete process.env.CRIXLY_SECRETS_MASTER_KEY_FILE;
      ...
      if (originalInlineMasterKey === undefined) {
        ...
        process.env.CRIXLY_SECRETS_MASTER_KEY = originalInlineMasterKey;
      }
      if (originalKeyFile === undefined) {
        delete process.env.CRIXLY_SECRETS_MASTER_KEY_FILE;
      } else {
        process.env.CRIXLY_SECRETS_MASTER_KEY_FILE = originalKeyFile;
      }
  ...
  it("writes the source inline secrets master key into the seeded worktree instance", () => {
    ...
    try {
      ...
      copySeededSecretsKey({
        ...
        sourceConfig: buildSourceConfig(),
        sourceEnvEntries: {
          CRIXLY_SECRETS_MASTER_KEY: "inline-source-master-key",
        },
        targetKeyFilePath: targetKeyPath,
      });
  ...
  it("persists the current agent jwt secret into the worktree env file", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "crixly-worktree-jwt-"));
    const repoRoot = path.join(tempRoot, "repo");
    const originalCwd = process.cwd();
    const originalJwtSecret = process.env.CRIXLY_AGENT_JWT_SECRET;

    try {
      fs.mkdirSync(repoRoot, { recursive: true });
      process.env.CRIXLY_AGENT_JWT_SECRET = "worktree-shared-secret";
      process.chdir(repoRoot);

      await worktreeInitCommand({
        seed: false,
        fromConfig: path.join(tempRoot, "missing", "config.json"),
        home: path.join(tempRoot, ".crixly-worktrees"),
      });

      const envPath = path.join(repoRoot, ".crixly", ".env");
      const envContents = fs.readFileSync(envPath, "utf8");
      expect(envContents).toContain("CRIXLY_AGENT_JWT_SECRET=worktree-shared-secret");
      expect(envContents).toContain("CRIXLY_WORKTREE_NAME=repo");
      expect(envContents).toMatch(/CRIXLY_WORKTREE_COLOR=\"#[0-9a-f]{6}\"/);
    } finally {
      process.chdir(originalCwd);
      if (originalJwtSecret === undefined) {
        delete process.env.CRIXLY_AGENT_JWT_SECRET;
      } else {
        process.env.CRIXLY_AGENT_JWT_SECRET = originalJwtSecret;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });

  it("defaults the seed source config to the current repo-local Crixly config", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "crixly-worktree-source-config-"));
    const repoRoot = path.join(tempRoot, "repo");
    const localConfigPath = path.join(repoRoot, ".crixly", "config.json");
    const originalCwd = process.cwd();
    const originalCrixlyConfig = process.env.CRIXLY_CONFIG;
    ...
    try {
      ...
      process.chdir(originalCwd);
      if (originalCrixlyConfig === undefined) {
        delete process.env.CRIXLY_CONFIG;
      } else {
        process.env.CRIXLY_CONFIG = originalCrixlyConfig;
      }
      fs.rmSync(tempRoot, { recursive: true, force: true });
    }
  });
  ...
  it("preserves the source config path across worktree:make cwd changes", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "crixly-worktree-source-override-"));
    const sourceConfigPath = path.join(tempRoot, "source", "config.json");
  ...
  it("rebinds same-repo workspace paths onto the current worktree root", () => {
    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/crixly",
        targetRepoRoot: "/Users/example/crixly-pr-432",
        workspaceCwd: "/Users/example/crixly",
      }),
    ).toBe("/Users/example/crixly-pr-432");

    expect(
      rebindWorkspaceCwd({
        sourceRepoRoot: "/Users/example/crixly",
        targetRepoRoot: "/Users/example/crixly-pr-432",
        workspaceCwd: "/Users/example/crixly/packages/db",
      }),
    ).toBe("/Users/example/crixly-pr-432/packages/db");
  });
  ...
  it("copies shared git hooks into a linked worktree git dir", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "crixly-worktree-hooks-"));
    const repoRoot = path.join(tempRoot, "repo");
  ...
  it("creates and initializes a worktree from the top-level worktree:make command", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "crixly-worktree-make-"));
    const repoRoot = path.join(tempRoot, "repo");
    const fakeHome = path.join(tempRoot, "home");
    const worktreePath = path.join(fakeHome, "crixly-make-test");
    const originalCwd = process.cwd();
    ...
    try {
      ...
      await worktreeMakeCommand("crixly-make-test", {
        seed: false,
        home: path.join(tempRoot, ".crixly-worktrees"),
      });

      expect(fs.existsSync(path.join(worktreePath, ".git"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".crixly", "config.json"))).toBe(true);
      expect(fs.existsSync(path.join(worktreePath, ".crixly", ".env"))).toBe(true);
    } finally {
      process.chdir(originalCwd);
