...
async function startTempDatabase() {
  const dataDir = mkdtempSync(path.join(os.tmpdir(), "crixly-company-cli-db-"));
  ...
  const { applyPendingMigrations, ensurePostgresDatabase } = await import("@crixlyai/db");
  const adminConnectionString = `postgres://crixly:crixly@127.0.0.1:${port}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "crixly");
  const connectionString = `postgres://crixly:crixly@127.0.0.1:${port}/crixly`;
...
function createServerEnv(configPath: string, port: number, connectionString: string) {
  ...
  env.CRIXLY_CONFIG = configPath;
  env.DATABASE_URL = connectionString;
  env.HOST = "127.0.0.1";
  env.PORT = String(port);
  env.SERVE_UI = "false";
  env.CRIXLY_DB_BACKUP_ENABLED = "false";
  env.HEARTBEAT_SCHEDULER_ENABLED = "false";
  env.CRIXLY_MIGRATION_AUTO_APPLY = "true";
  env.CRIXLY_UI_DEV_MIDDLEWARE = "false";
...
function createCliEnv() {
  ...
  delete env.HOST;
  delete env.SERVE_UI;
  delete env.CRIXLY_DB_BACKUP_ENABLED;
  delete env.HEARTBEAT_SCHEDULER_ENABLED;
  delete env.CRIXLY_MIGRATION_AUTO_APPLY;
  delete env.CRIXLY_UI_DEV_MIDDLEWARE;
  return env;
}
...
async function runCliJson<T>(args: string[], opts: { apiBase: string; configPath: string }) {
  ...
  const result = await execFileAsync(
    "pnpm",
    ["--silent", "crixlyai", ...args, "--api-base", opts.apiBase, "--config", opts.configPath, "--json"],
    {
      cwd: repoRoot,
      env: createCliEnv(),
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `crixlyai run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describe("crixlyai company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
  ...
  beforeAll(async () => {
    tempRoot = mkdtempSync(path.join(os.tmpdir(), "crixly-company-cli-e2e-"));
    ...
    const child = spawn(
      "pnpm",
      ["crixlyai", "run", "--config", configPath],
      {
        cwd: repoRoot,
  ...
  it("exports a company package and imports it into new and existing companies", async () => {
    ...
    expect(readFileSync(path.join(exportDir, ".crixly.yaml"), "utf8")).toContain('schema: "crixly/v1"');
    ...
    writeFileSync(zipPath, createStoredZipArchive(portableFiles, "crixly-demo"));
