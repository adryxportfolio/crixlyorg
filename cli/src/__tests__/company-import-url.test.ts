...
describe("isGithubShorthand", () => {
  it("matches owner/repo/path shorthands", () => {
    expect(isGithubShorthand("crixlyai/companies/gstack")).toBe(true);
    expect(isGithubShorthand("crixlyai/companies")).toBe(true);
  });
...
describe("normalizeGithubImportSource", () => {
  it("normalizes shorthand imports to canonical GitHub sources", () => {
    expect(normalizeGithubImportSource("crixlyai/companies/gstack")).toBe(
      "https://github.com/crixlyai/companies?ref=main&path=gstack",
    );
  });

  it("applies --ref to shorthand imports", () => {
    expect(normalizeGithubImportSource("crixlyai/companies/gstack", "feature/demo")).toBe(
      "https://github.com/crixlyai/companies?ref=feature%2Fdemo&path=gstack",
    );
  });
  ...
  it("applies --ref to existing GitHub tree URLs without losing the package path", () => {
    expect(
      normalizeGithubImportSource(
        "https://github.com/crixlyai/companies/tree/main/gstack",
        "release/2026-03-23",
      ),
    ).toBe(
      "https://github.com/crixlyai/companies?ref=release%2F2026-03-23&path=gstack",
    );
  });
});
