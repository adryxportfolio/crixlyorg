...
import { ApiConnectionError, ApiRequestError, CrixlyApiClient } from "../client/http.js";

describe("CrixlyApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new CrixlyApiClient({
    ...
    expect(headers["x-crixly-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new CrixlyApiClient({ apiBase: "http://localhost:3100" });
