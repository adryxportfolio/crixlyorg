...
export class CrixlyApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-crixly-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Crixly API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Crixly server is not running, the configured URL is wrong, or the request is being blocked before it reaches Crixly.",
    "",
    "Try:",
    "- Start Crixly with `pnpm dev` or `pnpm crixlyai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Crixly is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`CRIXLY_API_URL\`.`,
  );
  return lines.join("\n");
}
