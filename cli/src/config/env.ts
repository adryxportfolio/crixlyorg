...
const JWT_SECRET_ENV_KEY = "CRIXLY_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# Crixly environment variables",
    "# Generated by Crixly CLI commands",
...
export function resolveCrixlyEnvFile(configPath?: string): string {
...
export function loadCrixlyEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeCrixlyEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readCrixlyEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeCrixlyEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeCrixlyEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readCrixlyEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeCrixlyEnvEntries(next, filePath);
  return next;
}
