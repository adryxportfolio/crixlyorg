...
import { ensureAgentJwtSecret, loadCrixlyEnvFile, mergeCrixlyEnvEntries, readCrixlyEnvEntries, resolveCrixlyEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { CrixlyConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printCrixlyCliBanner } from "../utils/banner.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.CRIXLY_CONFIG;
...
const WORKTREE_NAME_PREFIX = "crixly-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.CRIXLY_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.CRIXLY_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromCrixlyConfig(config: CrixlyConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromCrixlyConfig(config);
}
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.crixly"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: CrixlyConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://crixly:crixly@127.0.0.1:${port}/crixly`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: CrixlyConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.CRIXLY_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.CRIXLY_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.CRIXLY_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.CRIXLY_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: CrixlyConfig;
  targetConfig: CrixlyConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveCrixlyEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readCrixlyEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://crixly:crixly@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "crixly");
    const targetConnectionString = `postgres://crixly:crixly@127.0.0.1:${targetHandle.port}/crixly`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readCrixlyEnvEntries(resolveCrixlyEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.CRIXLY_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.CRIXLY_AGENT_JWT_SECRET);
  mergeCrixlyEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { CRIXLY_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadCrixlyEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Crixly inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}

export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printCrixlyCliBanner();
  p.intro(pc.bgCyan(pc.black(" crixlyai worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printCrixlyCliBanner();
  p.intro(pc.bgCyan(pc.black(" crixlyai worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasCrixlyConfig: existsSync(path.resolve(worktreePath, ".crixly", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printCrixlyCliBanner();
  p.intro(pc.bgCyan(pc.black(" crixlyai worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveCrixlyEnvFile(configPath);
  const envEntries = readCrixlyEnvEntries(envPath);
  const out = {
    CRIXLY_CONFIG: configPath,
    ...(envEntries.CRIXLY_HOME ? { CRIXLY_HOME: envEntries.CRIXLY_HOME } : {}),
    ...(envEntries.CRIXLY_INSTANCE_ID ? { CRIXLY_INSTANCE_ID: envEntries.CRIXLY_INSTANCE_ID } : {}),
    ...(envEntries.CRIXLY_CONTEXT ? { CRIXLY_CONTEXT: envEntries.CRIXLY_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasCrixlyConfig)
      .map((choice) => path.resolve(choice.worktree, ".crixly", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readCrixlyEnvEntries(resolveCrixlyEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start Crixly once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasCrixlyConfig ? "crixly" : "no-crixly-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".crixly", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".crixly", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .crixly/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasCrixlyConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a Crixly worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasCrixlyConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No Crixly worktrees were found. Run `crixlyai worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Crixly configs are the same. Choose different --from/--to worktrees.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Crixly instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Crixly instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with crixly- if needed (created at ~/crixly-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: CRIXLY_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: CRIXLY_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source CRIXLY_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Crixly instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like Crixly worktrees")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with crixly- if needed")
