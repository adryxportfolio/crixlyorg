...
import { ApiRequestError, CrixlyApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Crixly config file")
    .option("-d, --data-dir <path>", "Crixly data directory root (isolates state from ~/.crixly)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Crixly API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.CRIXLY_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.CRIXLY_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.CRIXLY_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set CRIXLY_COMPANY_ID, or set context profile companyId via `crixlyai context set`.",
    );
  }

  const api = new CrixlyApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.CRIXLY_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.CRIXLY_SERVER_PORT || "");
