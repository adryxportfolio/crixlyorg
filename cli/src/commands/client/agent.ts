...
import type { Agent } from "@crixlyai/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveCrixlySkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export CRIXLY_API_URL='${escaped(input.apiBase)}'`,
    `export CRIXLY_COMPANY_ID='${escaped(input.companyId)}'`,
    `export CRIXLY_AGENT_ID='${escaped(input.agentId)}'`,
    `export CRIXLY_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Crixly skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Crixly skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveCrixlySkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Crixly skills directory. Expected ./skills in the repo checkout.",
              );
            }
