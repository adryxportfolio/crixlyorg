...
import { createDb, instanceUserRoles, invites } from "@crixlyai/db";
...
export async function bootstrapCeoInvite(opts: {
  ...
  const config = readConfig(configPath);
  if (!config) {
    p.log.error(`No config found at ${configPath}. Run ${pc.cyan("crixly onboard")} first.`);
    return;
  }
  ...
  try {
    ...
    p.log.info("If using embedded-postgres, start the Crixly server and run this command again.");
  } finally {
