...
type OnboardDefaults = Pick<CrixlyConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "CRIXLY_PUBLIC_URL",
  "DATABASE_URL",
  "CRIXLY_DB_BACKUP_ENABLED",
  "CRIXLY_DB_BACKUP_INTERVAL_MINUTES",
  "CRIXLY_DB_BACKUP_RETENTION_DAYS",
  "CRIXLY_DB_BACKUP_DIR",
  "CRIXLY_DEPLOYMENT_MODE",
  "CRIXLY_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "CRIXLY_ALLOWED_HOSTNAMES",
  "CRIXLY_AUTH_BASE_URL_MODE",
  "CRIXLY_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "CRIXLY_STORAGE_PROVIDER",
  "CRIXLY_STORAGE_LOCAL_DIR",
  "CRIXLY_STORAGE_S3_BUCKET",
  "CRIXLY_STORAGE_S3_REGION",
  "CRIXLY_STORAGE_S3_ENDPOINT",
  "CRIXLY_STORAGE_S3_PREFIX",
  "CRIXLY_STORAGE_S3_FORCE_PATH_STYLE",
  "CRIXLY_SECRETS_PROVIDER",
  "CRIXLY_SECRETS_STRICT_MODE",
  "CRIXLY_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolveCrixlyInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.CRIXLY_PUBLIC_URL?.trim() ||
    process.env.CRIXLY_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.CRIXLY_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.CRIXLY_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.CRIXLY_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.CRIXLY_ALLOWED_HOSTNAMES
    ? process.env.CRIXLY_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.CRIXLY_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.CRIXLY_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.CRIXLY_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.CRIXLY_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.CRIXLY_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.CRIXLY_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.CRIXLY_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.CRIXLY_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.CRIXLY_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.CRIXLY_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.CRIXLY_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.CRIXLY_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.CRIXLY_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.CRIXLY_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.CRIXLY_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "CRIXLY_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<CrixlyConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printCrixlyCliBanner();
  p.intro(pc.bgCyan(pc.black(" crixlyai onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveCrixlyInstanceId());
  p.log.message(
    pc.dim(
  ...
  let llm: CrixlyConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@crixlyai/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `crixlyai doctor`"));
      }
    }
  ...
  if (jwtSecret.created) {
    p.log.success(`Created ${pc.cyan("CRIXLY_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  } else if (process.env.CRIXLY_AGENT_JWT_SECRET?.trim()) {
    p.log.info(`Using existing ${pc.cyan("CRIXLY_AGENT_JWT_SECRET")} from environment`);
  } else {
    p.log.info(`Using existing ${pc.cyan("CRIXLY_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
  }

  const config: CrixlyConfig = {
    $meta: {
      version: 1,
  ...
  p.note(
    [
      ...
      "Agent auth: CRIXLY_AGENT_JWT_SECRET configured",
    ].join("\n"),
    "Configuration saved",
  );
  ...
  p.note(
    [
      `Run: ${pc.cyan("crixlyai run")}`,
      `Reconfigure later: ${pc.cyan("crixlyai configure")}`,
      `Diagnose setup: ${pc.cyan("crixlyai doctor")}`,
    ].join("\n"),
    "Next commands",
  );
  ...
  if (shouldRunNow && !opts.invokedByRun) {
    process.env.CRIXLY_OPEN_ON_LISTEN = "true";
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("crixlyai run")}`,
        `Then: ${pc.cyan("crixlyai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
