...
export const DEFAULT_WORKTREE_HOME = "~/.crixly-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".crixly");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: CrixlyConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "crixly",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    CRIXLY_HOME: paths.homeDir,
    CRIXLY_INSTANCE_ID: paths.instanceId,
    CRIXLY_CONFIG: paths.configPath,
    CRIXLY_CONTEXT: paths.contextPath,
    CRIXLY_IN_WORKTREE: "true",
    ...(branding?.name ? { CRIXLY_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { CRIXLY_WORKTREE_COLOR: branding.color } : {}),
  };
}
