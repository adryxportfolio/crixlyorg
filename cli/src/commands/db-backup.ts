...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@crixlyai/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://crixly:crixly@127.0.0.1:${port}/crixly`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" crixly db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveCrixlyInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "crixly";
