...
const DEFAULT_AGENT_JWT_ISSUER = "crixly";
const DEFAULT_AGENT_JWT_AUDIENCE = "crixly-api";
...
function defaultSecretsKeyFilePath(): string {
  return resolveDefaultSecretsKeyFilePath(resolveCrixlyInstanceId());
}
function defaultStorageBaseDir(): string {
  return resolveDefaultStorageDir(resolveCrixlyInstanceId());
}

export async function envCommand(opts: { config?: string }): Promise<void> {
  p.intro(pc.bgCyan(pc.black(" crixly env ")));

  const configPath = resolveConfigPath(opts.config);
  let config: CrixlyConfig | null = null;
...
function collectDeploymentEnvRows(config: CrixlyConfig | null, configPath: string): EnvVarRow[] {
  ...
  const publicUrlSource: EnvSource =
    process.env.CRIXLY_PUBLIC_URL
      ? "env"
      : process.env.CRIXLY_AUTH_PUBLIC_BASE_URL || process.env.BETTER_AUTH_URL || process.env.BETTER_AUTH_BASE_URL
        ? "env"
        : config?.auth?.publicBaseUrl
          ? "config"
          : "missing";
  ...
  const rows: EnvVarRow[] = [
    ...
    },
    {
      ...
      source: process.env.BETTER_AUTH_TRUSTED_ORIGINS
        ...
        : trustedOriginsDefault
          ...
          : "missing",
      required: false,
      note: "Comma-separated auth origin allowlist (auto-derived from CRIXLY_PUBLIC_URL when possible)",
    },
    {
      key: "CRIXLY_AGENT_JWT_TTL_SECONDS",
      value: process.env.CRIXLY_AGENT_JWT_TTL_SECONDS ?? DEFAULT_AGENT_JWT_TTL_SECONDS,
      source: process.env.CRIXLY_AGENT_JWT_TTL_SECONDS ? "env" : "default",
      required: false,
      note: "JWT lifetime in seconds",
    },
    {
      key: "CRIXLY_AGENT_JWT_ISSUER",
      value: process.env.CRIXLY_AGENT_JWT_ISSUER ?? DEFAULT_AGENT_JWT_ISSUER,
      source: process.env.CRIXLY_AGENT_JWT_ISSUER ? "env" : "default",
      required: false,
      note: "JWT issuer",
    },
    {
      key: "CRIXLY_AGENT_JWT_AUDIENCE",
      value: process.env.CRIXLY_AGENT_JWT_AUDIENCE ?? DEFAULT_AGENT_JWT_AUDIENCE,
      source: process.env.CRIXLY_AGENT_JWT_AUDIENCE ? "env" : "default",
      required: false,
      note: "JWT audience",
    },
    {
    ...
    {
      key: "CRIXLY_SECRETS_PROVIDER",
      value: secretsProvider,
      source: process.env.CRIXLY_SECRETS_PROVIDER
        ? "env"
        : config?.secrets?.provider
          ? "config"
          : "default",
    ...
    {
      key: "CRIXLY_SECRETS_STRICT_MODE",
      value: secretsStrictMode,
      source: process.env.CRIXLY_SECRETS_STRICT_MODE
        ? "env"
    ...
    {
      key: "CRIXLY_SECRETS_MASTER_KEY_FILE",
      value: secretsKeyFilePath,
      source: process.env.CRIXLY_SECRETS_MASTER_KEY_FILE
        ? "env"
        : config?.secrets?.localEncrypted?.keyFilePath
          ? "config"
          : "default",
    ...
    {
      key: "CRIXLY_STORAGE_PROVIDER",
      value: storageProvider,
      source: process.env.CRIXLY_STORAGE_PROVIDER
        ? "env"
        : config?.storage?.provider
          ? "config"
          : "default",
    ...
    {
      key: "CRIXLY_STORAGE_LOCAL_DIR",
      value: storageLocalDir,
      source: process.env.CRIXLY_STORAGE_LOCAL_DIR
        ? "env"
        : config?.storage?.localDisk?.baseDir
          ? "config"
          : "default",
    ...
    {
      key: "CRIXLY_STORAGE_S3_BUCKET",
      value: storageS3Bucket,
      source: process.env.CRIXLY_STORAGE_S3_BUCKET
        ? "env"
        : config?.storage?.s3?.bucket
          ? "config"
          : "default",
    ...
    {
      key: "CRIXLY_STORAGE_S3_REGION",
      value: storageS3Region,
      source: process.env.CRIXLY_STORAGE_S3_REGION
        ? "env"
        : config?.storage?.s3?.region
          ? "config"
          : "default",
    ...
    {
      key: "CRIXLY_STORAGE_S3_ENDPOINT",
      value: storageS3Endpoint,
      source: process.env.CRIXLY_STORAGE_S3_ENDPOINT
        ? "env"
        : config?.storage?.s3?.endpoint
          ? "config"
          : "default",
    ...
    {
      key: "CRIXLY_STORAGE_S3_PREFIX",
      value: storageS3Prefix,
      source: process.env.CRIXLY_STORAGE_S3_PREFIX
        ? "env"
        : config?.storage?.s3?.prefix
          ? "config"
          : "default",
    ...
    {
      key: "CRIXLY_STORAGE_S3_FORCE_PATH_STYLE",
      value: storageS3ForcePathStyle,
      source: process.env.CRIXLY_STORAGE_S3_FORCE_PATH_STYLE
        ? "env"
  ...
  if (process.env.CRIXLY_CONFIG || configPath !== defaultConfigPath) {
    rows.push({
      key: "CRIXLY_CONFIG",
      value: process.env.CRIXLY_CONFIG ?? configPath,
      source: process.env.CRIXLY_CONFIG ? "env" : "default",
      required: false,
