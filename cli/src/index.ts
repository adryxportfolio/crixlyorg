...
const DATA_DIR_OPTION_HELP =
  "Crixly data directory root (isolates state from ~/.crixly)";

program
  .name("crixlyai")
  .description("Crixly CLI — setup, diagnose, and configure your instance")
  .version("0.2.7");
...
program.hook("preAction", (_thisCommand, actionCommand) => {
  ...
  loadCrixlyEnvFile(options.config);
});
...
program
  ...
  .option("--run", "Start Crixly immediately after saving config", false)
  .action(onboard);

program
  .command("doctor")
  .description("Run diagnostic checks on your Crixly setup")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
program
  ...
  .option("--filename-prefix <prefix>", "Backup filename prefix", "crixly")
...
program
  .command("run")
  .description("Bootstrap local setup (onboard + doctor) and run Crixly")
  .option("-c, --config <path>", "Path to config file")
  .option("-d, --data-dir <path>", DATA_DIR_OPTION_HELP)
...
heartbeat
  ...
  .option("--api-base <url>", "Base URL for the Crixly server API")
