...
Example Crixly plugin that demonstrates:
...
```bash
pnpm --filter @crixlyai/plugin-file-browser-example build
pnpm crixlyai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm crixlyai plugin uninstall crixly-file-browser-example --force
```
...
- Optional: use `crixly-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
