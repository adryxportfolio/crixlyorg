...
type CrixlySprite = (typeof CRIXLY_SPRITES)[number];
...
function spriteSize(sprite: CrixlySprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = CRIXLY_SPRITES[(baseCol + baseRow) % CRIXLY_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = CRIXLY_SPRITES[Math.floor(Math.random() * CRIXLY_SPRITES.length)]!;
