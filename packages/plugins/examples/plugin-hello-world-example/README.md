# @crixlyai/plugin-hello-world-example
...
- a minimal React widget rendered in the Crixly dashboard
...
```bash
pnpm --filter @crixlyai/plugin-hello-world-example build
pnpm crixlyai plugin install ./packages/plugins/examples/plugin-hello-world-example
```
...
- **Reinstall after pulling.** If you installed a plugin by local path before the server stored `package_path`, the plugin may show status **error** (worker not found). Uninstall and install again so the server persists the path and can activate the plugin:  
  `pnpm crixlyai plugin uninstall crixly.hello-world-example --force` then  
  `pnpm crixlyai plugin install ./packages/plugins/examples/plugin-hello-world-example`.
