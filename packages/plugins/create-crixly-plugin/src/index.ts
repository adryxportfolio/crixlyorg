...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".crixly-sdk");
...
/**
 * Generate a complete Crixly plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Crixly plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "crixly-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["crixly", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@crixlyai/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@crixlyai/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@crixlyai/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@crixlyai/plugin-sdk\` and \`@crixlyai/shared\` from a local Crixly checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.crixly-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-crixly-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <crixly-sdk-path>]");
    process.exit(1);
  }
