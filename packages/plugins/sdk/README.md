...
Official TypeScript SDK for Crixly plugin authors.

- **Worker SDK:** `@crixlyai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@crixlyai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@crixlyai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@crixlyai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@crixlyai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@crixlyai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@crixlyai/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@crixlyai/plugin-sdk/ui/hooks` | Hooks only |
| `@crixlyai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@crixlyai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@crixlyai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@crixlyai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@crixlyai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@crixlyai/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main Crixly app. They can call ordinary Crixly HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Crixly instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@crixlyai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@crixlyai/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Crixly information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@crixlyai/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@crixlyai/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@crixlyai/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@crixlyai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@crixlyai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@crixlyai/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@crixlyai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@crixlyai/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@crixlyai/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@crixlyai/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@crixlyai/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@crixlyai/plugin-sdk/bundlers";
...
```bash
crixly-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@crixlyai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__crixly__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__crixly__/events` streams `reload` SSE events on UI build changes
