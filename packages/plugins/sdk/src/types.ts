/**
 * Core types for the Crixly plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@crixlyai/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @crixlyai/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Crixly object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Crixly UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Crixly domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Crixly domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Crixly secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Crixly secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@crixlyai/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@crixlyai/plugin-sdk";
 *
