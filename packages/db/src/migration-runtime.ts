...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://crixly:crixly@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "crixly");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://crixly:crixly@127.0.0.1:${preferredPort}/crixly`,
  ...
  if (runningPid) {
    ...
    return {
      connectionString: `postgres://crixly:crixly@127.0.0.1:${port}/crixly`,
  ...
  const adminConnectionString = `postgres://crixly:crixly@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "crixly");

  return {
    connectionString: `postgres://crixly:crixly@127.0.0.1:${selectedPort}/crixly`,
