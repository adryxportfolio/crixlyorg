...
function resolveCrixlyInstanceId(): string {
  const raw = process.env.CRIXLY_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveCrixlyHomeDir(), "instances", resolveCrixlyInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveCrixlyHomeDir(), "instances", resolveCrixlyInstanceId(), "data", "backups");
}
