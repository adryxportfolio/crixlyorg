...
async function createTempDatabase(): Promise<string> {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "crixly-db-client-"));
  ...
  const adminUrl = `postgres://crixly:crixly@127.0.0.1:${port}/postgres`;
  await ensurePostgresDatabase(adminUrl, "crixly");
