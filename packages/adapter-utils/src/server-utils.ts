...
const CRIXLY_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface CrixlySkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: CrixlySkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "crixly_required",
      originLabel: "Required by Crixly",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Crixly",
    readOnly: false,
  };
}
...
export function buildCrixlyEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const vars: Record<string, string> = {
    CRIXLY_AGENT_ID: agent.id,
    CRIXLY_COMPANY_ID: agent.companyId,
  };
  const runtimeHost = resolveHostForUrl(
    process.env.CRIXLY_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.CRIXLY_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.CRIXLY_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.CRIXLY_API_URL = apiUrl;
  return vars;
}
...
export async function resolveCrixlySkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...CRIXLY_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listCrixlySkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<CrixlySkillEntry[]> {
  const root = await resolveCrixlySkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `crixlyai/crixly/${entry.name}`,
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Crixly skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Crixly cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredCrixlyRuntimeSkills(value: unknown): CrixlySkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: CrixlySkillEntry[] = [];
...
export async function readCrixlyRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<CrixlySkillEntry[]> {
  const configuredEntries = normalizeConfiguredCrixlyRuntimeSkills(config.crixlyRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listCrixlySkillEntries(moduleDir, additionalCandidates);
}

export async function readCrixlySkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listCrixlySkillEntries(moduleDir);
...
export function readCrixlySkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.crixlySkillSync;
...
function canonicalizeDesiredCrixlySkillReference(
  reference: string,
...
export function resolveCrixlyDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readCrixlySkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredCrixlySkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeCrixlySkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.crixlySkillSync;
  const current =
  ...
  next.crixlySkillSync = current;
  return next;
}

export async function ensureCrixlySkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the Crixly server itself is started from
    // within a Claude Code session (e.g. `npx crixlyai run` in a terminal
