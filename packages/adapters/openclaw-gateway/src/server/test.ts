...
import { asString, parseObject } from "@crixlyai/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Crixly server host.",
        });
      }
    } catch (err) {
      checks.push({
