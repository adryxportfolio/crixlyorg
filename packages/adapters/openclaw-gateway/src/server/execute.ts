...
} from "@crixlyai/adapter-utils";
import { asNumber, asString, buildCrixlyEnv, parseObject } from "@crixlyai/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "crixly";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "crixly";
  if (input.strategy === "run") return `crixly:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `crixly:issue:${input.issueId}`;
  return fallback;
}
...
function resolveCrixlyApiUrlOverride(value: unknown): string | null {
...
function buildCrixlyEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const crixlyApiUrlOverride = resolveCrixlyApiUrlOverride(ctx.config.crixlyApiUrl);
  const crixlyEnv: Record<string, string> = {
    ...buildCrixlyEnv(ctx.agent),
    CRIXLY_RUN_ID: ctx.runId,
  };

  if (crixlyApiUrlOverride) {
    crixlyEnv.CRIXLY_API_URL = crixlyApiUrlOverride;
  }
  if (wakePayload.taskId) crixlyEnv.CRIXLY_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) crixlyEnv.CRIXLY_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) crixlyEnv.CRIXLY_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) crixlyEnv.CRIXLY_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) crixlyEnv.CRIXLY_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    crixlyEnv.CRIXLY_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return crixlyEnv;
}

function buildWakeText(payload: WakePayload, crixlyEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/crixly-claimed-api-key.json";
  const orderedKeys = [
    "CRIXLY_RUN_ID",
    "CRIXLY_AGENT_ID",
    "CRIXLY_COMPANY_ID",
    ...
    "CRIXLY_TASK_ID",
    "CRIXLY_WAKE_REASON",
    "CRIXLY_WAKE_COMMENT_ID",
    "CRIXLY_APPROVAL_ID",
    "CRIXLY_APPROVAL_STATUS",
    "CRIXLY_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = crixlyEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = crixlyEnv.CRIXLY_API_URL ?? "<set CRIXLY_API_URL>";

  const lines = [
    "Crixly wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `CRIXLY_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load CRIXLY_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $CRIXLY_API_KEY on every API call.",
    "- Use X-Crixly-Run-Id: $CRIXLY_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: CRIXLY_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$CRIXLY_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$CRIXLY_COMPANY_ID/issues?assigneeAgentId=$CRIXLY_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardCrixlyPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  crixlyEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateCrixly = parseObject(payloadTemplate.crixly);
  const workspace = asRecord(ctx.context.crixlyWorkspace);
  const workspaces = Array.isArray(ctx.context.crixlyWorkspaces)
    ? ctx.context.crixlyWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.crixlyRuntimeServiceIntents)
    ? ctx.context.crixlyRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardCrixly: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: crixlyEnv.CRIXLY_API_URL ?? null,
  };

  if (workspace) {
    standardCrixly.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardCrixly.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardCrixly.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "crixly-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const crixlyEnv = buildCrixlyEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, crixlyEnv);
  ...
  const crixlyPayload = buildStandardCrixlyPayload(ctx, wakePayload, crixlyEnv, payloadTemplate);
