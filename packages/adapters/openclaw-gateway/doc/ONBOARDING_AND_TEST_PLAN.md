...
## Scope
This plan is now **gateway-only**. Crixly supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Crixly skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Crixly task.
...
```bash
pnpm --filter @crixlyai/server test:run -- openclaw-gateway
pnpm --filter @crixlyai/server typecheck
pnpm --filter @crixlyai/ui typecheck
pnpm --filter crixlyai typecheck
```
