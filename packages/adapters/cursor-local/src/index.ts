...
Use when:
- You want Crixly to run Cursor Agent CLI locally as the agent runtime
...
- Crixly auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$crixly" and related skills on local runs.
- Crixly auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
