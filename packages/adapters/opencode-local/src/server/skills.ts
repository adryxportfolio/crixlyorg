...
async function buildOpenCodeSkillSnapshot(config: Record<string, unknown>): Promise<AdapterSkillSnapshot> {
  ...
  return buildPersistentSkillSnapshot({
    ...
    externalDetail: "Installed outside Crixly management in the shared skills home.",
    warnings: [
...
export async function syncOpenCodeSkills(
  ...
  const availableEntries = await readCrixlyRuntimeSkillEntries(ctx.config, __moduleDir);
  ...
  for (const available of availableEntries) {
    ...
    await ensureCrixlySkillSymlink(available.source, target);
  }
