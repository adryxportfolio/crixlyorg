...
import type { AdapterModel } from "@crixlyai/adapter-utils";
import { asString, runChildProcess } from "@crixlyai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.CRIXLY_PI_COMMAND === "string" &&
    process.env.CRIXLY_PI_COMMAND.trim().length > 0
      ? process.env.CRIXLY_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["CRIXLY_", "npm_", "NPM_"] as const;
