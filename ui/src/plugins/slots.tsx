 ...
 *
 * Plugin bundles are built with `external: ["@crixlyai/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__crixlyPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__crixlyPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__crixlyPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__crixlyPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@crixlyai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@crixlyai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@crixlyai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@crixlyai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@crixlyai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__crixlyPluginBridge__) {
 ...
 * self-register via `window.crixlyPlugins.registerReactComponent()`.
 ...
 * of `@crixlyai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
