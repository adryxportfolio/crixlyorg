...
import { buildAgentMentionHref, buildProjectMentionHref } from "@crixlyai/shared";
...
describe("MarkdownBody", () => {
  ...
  it("renders agent and project mentions as chips", () => {
    const html = renderToStaticMarkup(
      <ThemeProvider>
        <MarkdownBody>
          {`[@CodexCoder](${buildAgentMentionHref("agent-123", "code")}) [@Crixly App](${buildProjectMentionHref("project-456", "#336699")})`}
        </MarkdownBody>
      </ThemeProvider>,
    );
    ...
    expect(html).toContain("--crixly-mention-icon-mask");
    expect(html).toContain('href="/projects/project-456"');
    expect(html).toContain('data-mention-kind="project"');
    expect(html).toContain("--crixly-mention-project-color:#336699");
  });
});
