...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`crixly-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="crixly-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("crixly-mermaid-status", error && "crixly-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="crixly-mermaid-source">
...
export function MarkdownBody({ children, className, resolveImageSrc }: MarkdownBodyProps) {
      ...
      if (parsed) {
        ...
        return (
          <a
            ...
            className={cn(
              "crixly-mention-chip",
              `crixly-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "crixly-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "crixly-markdown prose prose-sm max-w-none break-words overflow-hidden",
