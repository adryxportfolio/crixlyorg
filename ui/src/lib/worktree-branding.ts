...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("crixly-worktree-enabled") !== "true") return null;

  const name = readMetaContent("crixly-worktree-name");
  const color = normalizeHexColor(readMetaContent("crixly-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("crixly-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
