...
export function OpenClawGatewayConfigFields({
  ...
  return (
    <>
      ...
      {!isCreate && (
        <>
          <Field label="Crixly API URL override">
            <DraftInput
              value={
                eff(
                  "adapterConfig",
                  "crixlyApiUrl",
                  String(config.crixlyApiUrl ?? ""),
                )
              }
              onCommit={(v) => mark("adapterConfig", "crixlyApiUrl", v || undefined)}
              immediate
              className={inputClass}
              placeholder="https://crixly.example"
            />
          </Field>
          ...
          {sessionStrategy === "fixed" && (
            <Field label="Session key">
              <DraftInput
                value={eff("adapterConfig", "sessionKey", String(config.sessionKey ?? "crixly"))}
          ...
          <Field label="Device auth">
            <div className="text-xs text-muted-foreground leading-relaxed">
              Always enabled for gateway agents. Crixly persists a device key during onboarding so pairing approvals
