...
/**
 * Filter .crixly.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterCrixlyYaml(yaml: string, checkedFiles: Set<string>): string {
...
export function CompanyExport() {
  ...
  // Recompute .crixly.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .crixly.yaml
    const yamlPath = exportData.crixlyExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterCrixlyYaml(exportData.files[yamlPath], checkedFiles);
    }
