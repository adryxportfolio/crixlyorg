...
Today release smoke testing for published Crixly packages is manual and shell-driven:
...
- runs `crixlyai@${CRIXLYAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@crixly.local`
- password: `crixly-smoke-password`
...
- the suite passes locally against both:
  - `CRIXLYAI_VERSION=canary`
  - `CRIXLYAI_VERSION=latest`
...
1. A published `crixlyai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `crixlyai@latest` Docker install can be smoke-tested by Playwright in CI.
