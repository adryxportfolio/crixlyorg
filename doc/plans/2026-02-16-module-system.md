...
Crixly's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Crixly's API, UI, and data model | Yes |
...
modules/
  observability/
    crixly.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@crixlyai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@crixlyai/core";
...
```
1. Scan modules/ directory for crixly.module.json manifests
...
```jsonc
// crixly.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@crixlyai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@crixlyai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:crixly/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.crixly.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm crixlyai store list                    # browse available modules and templates
pnpm crixlyai store install <module-id>     # install a module
pnpm crixlyai store import <template-id>    # import a company template
pnpm crixlyai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Crixly instance | `server:started` |
...
4. **Module config** — `crixly.config.json` with per-module config, env var interpolation
...
10. **`@crixlyai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
