...
Add a single storage subsystem for Crixly that supports:
...
- API serves attachment bytes through authenticated Crixly endpoints.
...
- Default local path is under instance root: `~/.crixly/instances/<instanceId>/data/storage`.
...
- `crixlyai onboard` writes a valid `storage` config block by default.
- `crixlyai configure --section storage` can switch between local and s3 modes.
...
- `crixlyai doctor` reports actionable storage status.
