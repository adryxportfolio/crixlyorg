...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Crixly plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Crixly.
...
- package: `@crixlyai/plugin-kitchen-sink-example`
- plugin id: `crixly.kitchen-sink-example` or `crixly-kitchen-sink-example`

Recommendation: use `crixly-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Crixly event such as `issue.created`
...
Recommendation: default to a Crixly-local endpoint or a stable public echo endpoint to avoid flaky docs.
