...
This document defines the next-stage plan for Crixly company import/export.

The core shift is:

- move from a Crixly-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Crixly.
...
Crixly already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Crixly
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `crixly.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.crixly.yaml` sidecar for high-fidelity Crixly-specific details
...
11. adapter-aware skill sync surfaces so Crixly can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Crixly at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Crixly sidecar.
...
Crixly must not redefine `SKILL.md`.
...
- Crixly-specific fidelity lives in `.crixly.yaml`
- Crixly may resolve and install `SKILL.md` packages, but it must not require a Crixly-only skill format
...
### 5.4 Base Package Vs Crixly Extension
...
- base package:
  ...
  - no Crixly-only runtime fields by default
- Crixly extension:
  - `.crixly.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Crixly tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`crixly.manifest.json` is not part of the future package direction.
...
Crixly import/export should support these entity kinds:
...
- Crixly can treat “this set of agents came from team package X” as the imported-team model
...
Crixly should support source references in package metadata with:
...
Crixly should define a new adapter surface area around skills:
...
Planned Crixly behavior:

- if an adapter supports read, Crixly should show current skills in the UI
- if an adapter supports write, Crixly should let the user enable/disable imported skills
- if an adapter supports sync, Crixly should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.crixly.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.crixly.yaml` alongside the base package
...
## 10. Storage Model Inside Crixly
...
Crixly should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `crixlyai company export <company-id> --out <path>`
- `crixlyai company import <path-or-url> --dry-run`
- `crixlyai company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Crixly storage, or only referenced at import time?
...
3. Should Crixly support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Crixly aligned with:
