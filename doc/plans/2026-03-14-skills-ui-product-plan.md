...
This document defines the product and UI plan for skill management in Crixly.
...
- Crixly company import/export can include skills as package content
...
3. The UI must always tell the truth about what Crixly knows:
   - desired state in Crixly
...
Crixly should treat skills at two scopes:
...
- created locally inside Crixly later
...
7. “Understand what Crixly knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Crixly-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Crixly did not install.

These should be clearly marked:

- external
- not managed by Crixly
...
- `desired_only`: Crixly wants it, adapter does not show it yet
- `external`: adapter has it but Crixly does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Crixly only tracks desired state
...
- Crixly can still track desired skills
...
- `.crixly.yaml` may add Crixly-specific fidelity, but should not replace the base shortname association model
...
- `.crixly.yaml` may add Crixly-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Crixly should have a company-scoped skill model or managed package model representing:
...
Crixly should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Crixly-specific.
...
That gives Crixly one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
