...
- local mode that still feels instant (`npx crixlyai run` and go)
...
2. CLI command `pnpm crixlyai auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm crixlyai onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm crixlyai onboard`).
...
- expose key instance settings in API + CLI (`crixlyai configure` / `crixlyai onboard`)
...
7. `pnpm crixlyai onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
