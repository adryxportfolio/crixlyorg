...
This document defines the rollout plan for adapter-wide skill support in Crixly.
...
- Crixly stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Crixly currently has these adapters:
...
1. Desired skills live in Crixly for every adapter.
...
These adapters have a stable local skills directory that Crixly can read and manage.
...
These adapters do not have a meaningful Crixly-owned persistent install state.
...
- desired skills stored in Crixly
...
- runtime already injects Crixly skills into `~/.cursor/skills`
...
- runtime already injects Crixly skills into `~/.gemini/skills`
...
- runtime already injects Crixly skills into `~/.pi/agent/skills`
...
- Crixly can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Crixly skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Crixly may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Crixly skills that are clearly marked as Crixly-managed
...
- Crixly stores desired skills only
...
- avoid destructive wording unless Crixly can prove a skill is Crixly-managed
...
That gets Crixly from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
