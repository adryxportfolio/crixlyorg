...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/crixly-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `CRIXLY_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `CRIXLY_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Crixly may create host-local worktrees, processes, and paths
- `cloud`
  - Crixly should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `crixly | github | vercel | s3 | custom`
...
- use `CRIXLY_EXECUTION_TOPOLOGY` as a defaulting guardrail
