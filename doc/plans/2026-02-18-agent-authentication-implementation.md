...
- New config/env settings:
  - `CRIXLY_AGENT_JWT_SECRET`
  - `CRIXLY_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `CRIXLY_AGENT_JWT_ISSUER` (default: `crixly`)
  - `CRIXLY_AGENT_JWT_AUDIENCE` (default: `crixly-api`)
...
1. In:
   ...
   inject `CRIXLY_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `CRIXLY_API_KEY`, do not overwrite it.
- Continue injecting:
  - `CRIXLY_AGENT_ID`
  - `CRIXLY_COMPANY_ID`
  - `CRIXLY_API_URL`
...
- Local adapters authenticate without manual `CRIXLY_API_KEY` config.
