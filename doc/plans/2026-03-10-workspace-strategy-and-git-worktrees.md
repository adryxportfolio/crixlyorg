...
`PAP-447` asks how Crixly should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Crixly instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Crixly:

- some operators use Crixly to manage Crixly and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Crixly
- Claude and Codex expose different built-in affordances, so Crixly should not overfit to one tool

## Core Product Decision

Crixly should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Crixly
...
- Crixly should not assume that every adapter can see or use a host filesystem path directly
...
If Crixly makes worktrees agent-first, it will blur:
...
- Crixly core resolves and records execution workspace state
...
That gives Crixly room to support:
...
- Crixly handles workspace runtime services under the hood using project/workspace policy
...
Once Crixly is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Crixly cannot infer
...
- when issue moves to done, should Crixly auto-commit?
- should it auto-push?
...
Crixly already has the right foundation for a project-first model.
...
For server-spawned local adapters, Crixly already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Crixly should mirror directly

Implication:

- for `codex_local`, Crixly should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Crixly should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Crixly should separate:
...
Crixly should be able to express intentions such as:
...
These are not specific to Crixly. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".crixly/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Crixly-specific `devServer` flag.
...
Crixly should distinguish between:
...
Crixly should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Crixly should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Crixly
...
For runtime services specifically, Crixly should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Crixly-only dev-server flags
...
3. Inject resolved execution workspace details into `context.crixlyWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Crixly instance can be reused through env/config injection when appropriate
...
- Crixly can decide whether to reuse or start a fresh service deterministically
...
3. Test against Crixly-on-Crixly workflows first.
...
7. Workspace runtime services are modeled generically, not as Crixly-specific dev-server toggles.
