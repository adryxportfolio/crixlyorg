...
Crixly needs to support two very different but equally valid ways of working:
...
Today, Crixly already has the beginnings of this model:
...
- making Crixly feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Crixly users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Crixly deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Crixly into a full code review product
...
This object must be recorded explicitly so that Crixly can:
...
Crixly should treat PRs as a type of work product linked back to:
...
Crixly already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Crixly must be a first-class deployment mode

Crixly cannot assume that it is running on the same machine as the code.

In cloud deployments, Crixly may:
...
- `Runtime service`: a process or service Crixly owns or tracks for a workspace
...
It also lets cloud-hosted Crixly deployments point at codebases and remotes without pretending the Crixly host has direct filesystem access.
...
Without an explicit `execution workspace` record, Crixly has nowhere stable to attach:
...
Crixly needs a single place to show:
...
- Crixly-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Crixly owning the execution host
...
## 1. Crixly host and execution host must be decoupled
...
without the Crixly host itself running local git or local preview processes.
...
Crixly should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Crixly.
...
Crixly should keep workspace automation centered on a small set of hooks:
...
Crixly should provide the equivalent visibility in the project `Code` surface:
...
Crixly should treat execution workspaces as the natural home for:
...
Crixly should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Crixly should continue treating cleanup policy as part of the core workspace model:
...
Crixly should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Crixly host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Crixly should borrow Worktrunk's good execution patterns while keeping the broader Crixly model:
...
That keeps Crixly focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Crixly deployments remain first-class

That is a better fit for Crixly than either extreme:
