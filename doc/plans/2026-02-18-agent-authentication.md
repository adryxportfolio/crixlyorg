...
Agents need API keys to authenticate with Crixly. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Crixly
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Crixly generates a token and passes it directly to the agent
...
`CRIXLY_API_URL`, `CRIXLY_AGENT_ID`, `CRIXLY_COMPANY_ID`. We need to
add a `CRIXLY_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Crixly CLI
...
```
crixly auth login
# Opens browser -> user confirms -> token stored at ~/.crixly/credentials
```
...
managed by the Crixly server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Crixly.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Crixly SKILL.md (or a link to it)
   - What information Crixly needs from the agent (e.g. webhook URL, adapter
...
5. Crixly stores the pending registration.
...
7. On approval, Crixly provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Crixly issues an API key (or JWT) upon approval, delivered
...
1. Generate a Crixly invite link for the company.
...
4. A Crixly company member approves the new agent.
5. Crixly begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `CRIXLY_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `crixlyai auth login` for developer-managed remote agents.                                      |
