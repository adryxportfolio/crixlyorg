...
Crixly already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Crixly should implement next.

## Product Goals

Crixly should let operators:
...
For the next implementation pass, Crixly should enforce these defaults:
...
Crixly should separate subscription-included usage from billed spend:
...
Crixly should have two threshold classes:
...
Crixly must know that a project is budget-paused and enforce it.
...
Crixly must also block execution before new work starts.
