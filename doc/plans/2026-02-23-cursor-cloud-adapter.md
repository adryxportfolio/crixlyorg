...
This document defines the V1 design for a Crixly adapter that integrates with
...
4. synthesized stdout events for Crixly UI/CLI
...
3. **Agent callback auth to Crixly** uses a bootstrap exchange flow (no long-lived Crixly key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Crixly endpoints, not full SKILL.md prompt injection.
...
- `crixlyPublicUrl` (optional override; else `CRIXLY_PUBLIC_URL` env)
...
## Crixly Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `CRIXLY_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.crixlyPublicUrl`
2. `process.env.CRIXLY_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Crixly credentials in prompt text.

Flow:

1. Before launch/follow-up, Crixly mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Crixly validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Crixly API calls.
...
1. Prompt includes a compact instruction to fetch skills from Crixly.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/crixly`
   - `GET /api/skills/crixly-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <crixlyPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Crixly run context.
...
5. `crixlyPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Crixly skill endpoints |
| Crixly auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Crixly context.
3. Add richer PR metadata surfacing in Crixly UI.
