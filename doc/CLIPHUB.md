...
ClipHub is the public registry where people share, discover, and download Crixly company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Crixly what a package registry is to a programming language. Crixly already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Crixly — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Crixly instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Crixly spec. It contains:
...
**Install (fresh start):**
```
crixly install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Crixly instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Crixly, export your company as a template, then publish:

```
crixly export --template my-company
crixly publish cliphub my-company
```
...
ClipHub is a **separate service** from Crixly itself. Crixly is self-hosted; ClipHub is a hosted registry that Crixly instances talk to.
...
| **Crixly CLI** | `crixlyai install`, `crixlyai publish`, `crixlyai cliphub sync` — built into Crixly |
| **Crixly UI** | "Browse ClipHub" panel in the Crixly web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Crixly) |
| Backend | TypeScript + Hono (consistent with Crixly) |
| Database | PostgreSQL |
...
4. Run `crixlyai install cliphub:acme/lean-saas-shop`
5. Crixly creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Crixly until it works well
2. Export: `crixlyai export --template my-agency`
3. Publish: `crixlyai publish cliphub my-agency`
...
3. Install just that agent: `crixlyai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Crixly

ClipHub is **not required** to use Crixly. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Crixly what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `crixlyai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Crixly web UI
- [ ] `crixlyai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Crixly's own philosophy)
