...
This guide describes the current, implemented way to create a Crixly plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Crixly app.
...
```bash
pnpm --filter @crixlyai/create-crixly-plugin build
node packages/plugins/create-crixly-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Crixly repo:

```bash
pnpm --filter @crixlyai/create-crixly-plugin build
node packages/plugins/create-crixly-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@crixlyai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@crixlyai/plugin-sdk` from the local Crixly checkout into a `.crixly-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Crixly from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@crixlyai/plugin-sdk/ui`
