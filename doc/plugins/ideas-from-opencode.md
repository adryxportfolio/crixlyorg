...
Crixly V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Crixly-shaped design.

Assumption for this document: Crixly is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Crixly objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Crixly.

The main conclusion is:

- Crixly should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Crixly should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Crixly should use multiple extension classes instead of one generic plugin bag:
...
If Crixly does this well, the examples you listed become straightforward:
...
Relevant Crixly files reviewed for current extension seams:
...
It is too dangerous for Crixly core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Crixly — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Crixly should absolutely do this.
...
Crixly should copy this discipline.
...
Crixly should adopt this style for plugin actions, automations, and UI schemas.
...
Crixly can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Crixly too.

## What Crixly Should Not Copy Directly
...
Crixly is a control plane for an operator-managed instance with company objects.
...
Crixly is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Crixly should not auto-load arbitrary code from a workspace repo like `.crixly/plugins` or project directories.
...
For Crixly, equivalent hooks into:
...
Crixly should not allow plugins to silently replace:
...
For Crixly it would be risky because it combines:
...
Crixly should require an explicit operator install step.

## Why Crixly Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Crixly |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Crixly instance |
...
That means Crixly should borrow the good ideas from `opencode` but use a stricter architecture.

## Crixly Already Has Useful Pre-Plugin Seams

Crixly has several extension-like seams already:
...
This is good news.
Crixly does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Crixly Plugin Model
...
Crixly already has this pattern implicitly:
...
- `plugin` means integration code that talks to Crixly through a typed plugin protocol and capability model
...
- invoke explicit Crixly actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@crixlyai/plugin-sdk/ui`) with design tokens and common components.
...
Crixly should treat plugin installation as a global instance-level action.

Examples:

- install `@crixly/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Crixly objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Crixly already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Crixly too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@crixly/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@crixlyai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Crixly instance.

A `create-crixly-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Crixly achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Crixly server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Crixly's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@crixlyai/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@crixlyai/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Crixly

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@crixlyai/plugin-sdk";

export default definePlugin({
  id: "@crixly/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Crixly entities
    });
    ...
    ctx.events.on("plugin.@crixly/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@crixlyai/plugin-sdk/ui";
...
Any Crixly plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Crixly objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@crixly/plugin-linear`)
...
- reuse core tables when the data is clearly part of Crixly itself
...
If a concept is becoming part of Crixly's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Crixly itself
...
### 3. Scope plugin data to Crixly objects before adding custom schemas

A lot of plugin data naturally hangs off existing Crixly objects:
...
If Crixly eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@crixly/plugin-workspace-files`
...
Package idea: `@crixly/plugin-terminal`
...
Package idea: `@crixly/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Crixly.
- Board reviews the diff after a run without leaving Crixly.
...
- optional `events.emit` (e.g. `plugin.@crixly/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@crixly/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@crixly/plugin-linear`

This plugin syncs Crixly work with Linear. It is useful for:

- importing backlog from Linear
- linking Crixly issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Crixly and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Crixly.
- Crixly issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@crixly/plugin-github-issues`

This plugin syncs Crixly issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Crixly issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Crixly task governance
...
- Plugin settings:
  ...
  - whether new Crixly issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Crixly issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Crixly.
- GitHub webhooks update status/comment state in Crixly.
- A PR is linked back to the Crixly issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@crixly/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@crixly/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Crixly. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Crixly issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Crixly plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Crixly dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Crixly issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Crixly core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@crixly/plugin-runtime-processes`
...
Package idea: `@crixly/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Crixly. It is useful for:
...
- Failed payment spikes or churn events can generate Crixly issues for follow-up.
...
- Stripe data should stay additive to Crixly core
...
- plugin UI bundle loading, host bridge, `@crixlyai/plugin-sdk/ui`
...
- `@crixlyai/plugin-test-harness` and `create-crixly-plugin` starter template
...
Crixly should not implement "an OpenCode-style generic in-process hook system."
Crixly should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Crixly
...
8. Ship `@crixlyai/plugin-test-harness` and `create-crixly-plugin` starter template.
