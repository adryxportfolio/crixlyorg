# Crixly Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Crixly's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Crixly app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Crixly HTTP APIs directly.
...
Crixly plugin design is based on the following assumptions:

1. Crixly is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Crixly business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Crixly core.
...
2. Let plugins add major capabilities without editing Crixly core.
...
4. Depend on project-local plugin folders such as `.crixly/plugins`.
...
The single Crixly deployment an operator installs and controls.

### 5.2 Company

A first-class Crixly business object inside the instance.
...
A trusted in-process extension loaded directly by Crixly core.
...
An installable instance-wide extension package loaded through the Crixly plugin runtime.
...
Crixly has two extension classes.
...
Crixly already has a concrete workspace model:
...
Plugins live under the Crixly instance directory.

Suggested layout:

- `~/.crixly/instances/default/plugins/package.json`
- `~/.crixly/instances/default/plugins/node_modules/`
- `~/.crixly/instances/default/plugins/.cache/`
- `~/.crixly/instances/default/data/plugins/<plugin-id>/`
...
Crixly should add CLI commands:

- `pnpm crixlyai plugin list`
- `pnpm crixlyai plugin install <package[@version]>`
- `pnpm crixlyai plugin uninstall <plugin-id>`
- `pnpm crixlyai plugin upgrade <plugin-id> [version]`
- `pnpm crixlyai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@crixly/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface CrixlyPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
...
Plugins may contribute tools that Crixly agents can use during runs.
...
- Crixly server starts one worker process per installed plugin
...
Receives one typed Crixly domain event.
...
export function definePlugin(definition: PluginDefinition): CrixlyPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: CrixlyPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@crixlyai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@crixlyai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@crixlyai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@crixlyai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@crixlyai/plugin-sdk/ui`
...
The `@crixlyai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Crixly secret provider system rather than a plain text input
...
1. Core Crixly data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Crixly objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Crixly product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Crixly later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Crixly secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm crixlyai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Crixly server. This is a normative requirement, not optional.
...
### 27.1 `@crixlyai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@crixlyai/plugin-test-harness";
...
For developing a plugin against a running Crixly instance:

- The operator installs the plugin from a local path: `pnpm crixlyai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-crixly-plugin`) that scaffolds:

- `package.json` with correct `crixlyPlugin` keys
...
- `@crixly/plugin-workspace-files`
- `@crixly/plugin-terminal`
- `@crixly/plugin-git`
- `@crixly/plugin-linear`
- `@crixly/plugin-github-issues`
- `@crixly/plugin-grafana`
- `@crixly/plugin-runtime-processes`
- `@crixly/plugin-stripe`
...
- `@crixlyai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@crixlyai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@crixlyai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@crixlyai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@crixlyai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@crixlyai/plugin-sdk/ui`
...
- `@crixlyai/plugin-test-harness`
- `create-crixly-plugin` starter template
...
Crixly should not implement a generic in-process hook bag modeled directly after local coding tools.

Crixly should implement:
...
That is the complete target design for the Crixly plugin system.
