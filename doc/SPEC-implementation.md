...
Crixly V1 must provide a full control-plane loop for autonomous agents:
...
- Primary: PostgreSQL
- Local default: embedded PostgreSQL at `~/.crixly/instances/default/db`
- Optional local prod-like: Docker Postgres
- Optional hosted: Supabase/Postgres-compatible
- File/object storage:
  - local default: `~/.crixly/instances/default/data/storage` (`local_disk`)
...
- `.crixly.yaml` sidecar for Crixly-specific fidelity
...
- export emits a clean vendor-neutral markdown package plus `.crixly.yaml`
- projects and starter tasks are opt-in export content rather than default package content
- recurring `TASK.md` entries use `recurring: true` in the base package and Crixly routine fidelity in `.crixly.yaml`
- Crixly imports recurring task packages as routines instead of downgrading them to one-time issues
- export strips environment-specific paths (`cwd`, local instruction file paths, inline prompt duplication) while preserving portable project repo/workspace metadata such as `repoUrl`, refs, and workspace-policy references keyed in `.crixly.yaml`
