...
This spec defines how Crixly actually runs agents while staying runtime-agnostic.
...
1. Crixly is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Crixly manages lifecycle, and on exit Crixly parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `crixly.skill` (shared Crixly skill text block)
...
2. Preferred transport is env vars (`CRIXLY_*`) injected at runtime.
