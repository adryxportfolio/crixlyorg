...
Maintainer runbook for shipping Crixly across npm, GitHub, and the website-facing changelog surface.
...
Crixly uses calendar versions that still fit semver syntax:
...
2. **npm** — `crixlyai` and public workspace packages are published
...
```bash
npx crixlyai@canary onboard
# or
npx crixlyai@canary onboard --data-dir "$(mktemp -d /tmp/crixly-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/crixlyai/crixly/actions/workflows/release.yml)
...
```bash
CRIXLYAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

For the current stable:

```bash
CRIXLYAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Useful isolated variants:

```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary CRIXLYAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable CRIXLYAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Automated browser smoke is also available:

```bash
gh workflow run release-smoke.yml -f crixly_version=canary
gh workflow run release-smoke.yml -f crixly_version=latest
```

Minimum checks:

- `npx crixlyai@canary onboard` installs
