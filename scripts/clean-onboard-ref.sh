...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Crixly data dir to use
  CRIXLY_HOST=127.0.0.1    Host passed to the onboarded server
  CRIXLY_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/crixly-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/crixly-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export CRIXLY_HOST="${CRIXLY_HOST:-127.0.0.1}"
export CRIXLY_PORT="${CRIXLY_PORT:-3100}"
export CRIXLY_OPEN_ON_LISTEN="${CRIXLY_OPEN_ON_LISTEN:-false}"
...
echo "CRIXLY_HOST: $CRIXLY_HOST"
echo "CRIXLY_PORT: $CRIXLY_PORT"
...
env \
  ...
  HOST="$CRIXLY_HOST" \
  PORT="$CRIXLY_PORT" \
  CRIXLY_OPEN_ON_LISTEN="$CRIXLY_OPEN_ON_LISTEN" \
  pnpm crixlyai onboard --yes --data-dir "$PC_DATA"
