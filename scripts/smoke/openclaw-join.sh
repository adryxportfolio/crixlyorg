...
COMPANY_ID="${COMPANY_ID:-${CRIXLY_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-crixly-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-crixly-openclaw-smoke}"
...
Provide one of:
  CRIXLY_AUTH_HEADER=\"Bearer <board-token>\"
  CRIXLY_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Crixly OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.crixly.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
