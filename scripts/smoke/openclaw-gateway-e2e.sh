...
CRIXLY_API_URL="${CRIXLY_API_URL:-http://127.0.0.1:3100}"
API_BASE="${CRIXLY_API_URL%/}/api"
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-crixly-smoke}"
...
CRIXLY_API_URL_FOR_OPENCLAW="${CRIXLY_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${CRIXLY_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${CRIXLY_AUTH_HEADER}" )
fi
if [[ -n "${CRIXLY_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${CRIXLY_COOKIE}" )
  CRIXLY_BROWSER_ORIGIN="${CRIXLY_BROWSER_ORIGIN:-${CRIXLY_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${CRIXLY_BROWSER_ORIGIN}" -H "Referer: ${CRIXLY_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set CRIXLY_COOKIE or CRIXLY_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg crixlyApiUrl "$CRIXLY_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/crixly"
  local claimed_file="${workspace_dir}/crixly-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/crixly-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/crixly-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/crixly-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/crixly"
    assert_status "200"
    {
      echo "CRIXLY_API_URL=${CRIXLY_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set CRIXLY_API_KEY=${AGENT_API_KEY} in your run context before running Crixly heartbeat steps."
...
main() {
  ...
  wait_http_ready "${CRIXLY_API_URL%/}/api/health" 15 || fail "Crixly API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "crixly health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
