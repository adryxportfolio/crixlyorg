...
CRIXLY_HOST_PORT="${CRIXLY_HOST_PORT:-3100}"
CRIXLY_HOST_FROM_CONTAINER="${CRIXLY_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.crixly-openclaw.override.yml"
...
detect_crixly_base_url() {
  ...
  for candidate in "$CRIXLY_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${CRIXLY_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${CRIXLY_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
crixly_base_url="$(detect_crixly_base_url || true)"
...
Crixly URL for OpenClaw container:
EOF
  if [[ -n "$crixly_base_url" ]]; then
    cat <<EOF
  $crixly_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${CRIXLY_HOST_PORT}
  (Do not use http://127.0.0.1:${CRIXLY_HOST_PORT} inside the container.)
  If Crixly rejects the host, run on host machine:
    pnpm crixlyai allowed-hostname host.docker.internal
  Then restart Crixly and re-run this script.
EOF
  fi
  cat <<EOF
