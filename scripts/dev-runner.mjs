...
const devServerStatusFilePath = path.join(repoRoot, ".crixly", "dev-server-status.json");
...
const env = {
  ...process.env,
  CRIXLY_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.CRIXLY_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
}

if (mode === "watch") {
  env.CRIXLY_MIGRATION_PROMPT ??= "never";
  env.CRIXLY_MIGRATION_AUTO_APPLY ??= "true";
}

if (tailscaleAuth) {
  env.CRIXLY_DEPLOYMENT_MODE = "authenticated";
  env.CRIXLY_DEPLOYMENT_EXPOSURE = "private";
  env.CRIXLY_AUTH_BASE_URL_MODE = "auto";
  env.HOST = "0.0.0.0";
  console.log("[crixly] dev mode: authenticated/private (tailscale-friendly) on 0.0.0.0");
} else {
  console.log("[crixly] dev mode: local_trusted (default)");
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@crixlyai/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[crixly] Command failed with code ${status.code}: pnpm --filter @crixlyai/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[crixly] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options = {}) {
  ...
  const autoApply = options.autoApply ?? env.CRIXLY_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[crixly] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). ` +
...
async function buildPluginSdk() {
  console.log("[crixly] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@crixlyai/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[crixly] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@crixlyai/server", serverScript, ...forwardedArgs],
