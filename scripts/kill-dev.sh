#!/usr/bin/env bash
#
# Kill all local Crixly dev server processes (across all worktrees).
#
# Usage:
#   scripts/kill-dev.sh        # kill all crixly dev processes
...
# Collect PIDs of node processes running from any crixly directory.
# Matches paths like /Users/*/crixly/... or /Users/*/crixly-*/...
...
while IFS= read -r line; do
  ...
  pids+=("$pid")
  lines+=("$line")
done < <(ps aux | grep -E '/crixly(-[^/]+)?/' | grep node | grep -v grep || true)

if [[ ${#pids[@]} -eq 0 ]]; then
  echo "No Crixly dev processes found."
  exit 0
fi

echo "Found ${#pids[@]} Crixly dev process(es):"
echo ""
