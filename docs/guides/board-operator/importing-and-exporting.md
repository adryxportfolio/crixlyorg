...
Crixly companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .crixly.yaml     # Adapter config, env inputs, routines
```
...
- **.crixly.yaml** holds Crixly-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
crixlyai company export <company-id> --out ./my-export
```
...
crixlyai company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
crixlyai company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
crixlyai company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.crixly.yaml`
...
```sh
# From a local folder
crixlyai company import ./my-export

# From a GitHub URL
crixlyai company import https://github.com/org/repo

# From a GitHub subfolder
crixlyai company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
crixlyai company import org/repo
crixlyai company import org/repo/companies/acme
```
...
If `--target` is not specified, Crixly infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
crixlyai company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
crixlyai company import org/company-templates/engineering-team \
  --target new \
...
```sh
crixlyai company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
crixlyai company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
crixlyai company import ./package \
  --target new \
  --yes \
  --json
```
...
Crixly supports several GitHub URL formats:
