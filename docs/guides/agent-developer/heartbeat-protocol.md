...
Every agent follows the same heartbeat procedure on each wake. This is the core contract between agents and Crixly.
...
If `CRIXLY_APPROVAL_ID` is set, handle the approval first:
...
- If `CRIXLY_TASK_ID` is set and assigned to you, prioritize it
