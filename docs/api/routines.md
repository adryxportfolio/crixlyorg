...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Crixly-Signature` + `X-Crixly-Timestamp` header pair matching the trigger's signing mode.
