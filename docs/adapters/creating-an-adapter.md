...
Build a custom adapter to connect Crixly to any agent runtime.
...
2. Build environment with `buildCrixlyEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `crixlyai run --watch` using `picocolors`.
...
Make Crixly skills discoverable to your agent runtime without writing to the agent's working directory:
