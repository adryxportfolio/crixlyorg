...
Crixly is a monorepo with four main layers.
...
│
├── skills/                      # Agent skills
│   └── crixly/               # Core Crixly skill (heartbeat protocol)
│
├── cli/                         # CLI client
...
3. **Agent process** — Adapter spawns the agent (e.g. Claude Code CLI) with Crixly env vars and a prompt
4. **Agent work** — The agent calls Crixly's REST API to check assignments, checkout tasks, do work, and update status
...
Adapters are the bridge between Crixly and agent runtimes. Each adapter is a package with three modules:
...
- **CLI module** — terminal formatter for `crixlyai run --watch`

Built-in adapters: `claude_local`, `codex_local`, `process`, `http`. You can create custom adapters for any runtime.

## Key Design Decisions

- **Control plane, not execution plane** — Crixly orchestrates agents; it doesn't run them
