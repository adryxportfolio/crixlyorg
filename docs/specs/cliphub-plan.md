# ClipHub: Marketplace for Crixly Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Crixly configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Crixly-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Crixly company (agents, projects, governance) |
...
A complete Crixly company configuration:
...
Single-agent configurations designed to plug into a Crixly org:
...
Portable skill files that any Crixly agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Crixly's skill loading system
...
interface Listing {
  ...
  crixlyVersionMin: string;     // Minimum Crixly version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Crixly company
...
| `POST` | `/api/listings/:id/install` | Install to Crixly company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Crixly company (or create new)
  → ClipHub API calls Crixly API to:
  ...
  → Redirect to Crixly dashboard with new team running
```
...
### 5.3 Creator: Export from Crixly → Publish

```
Running Crixly company → "Export as Blueprint" (CLI or UI)
  → Crixly exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Crixly brand blue/purple)
...
| Install flow | One-click deploy to Crixly company |
...
{
  "targetCompanyId": "uuid",         // Existing Crixly company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `crixly-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Crixly
- **Backend**: Node.js API (or extend Crixly server)
- **Database**: Postgres (can share Crixly's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Crixly auth (or OAuth2)

### 9.2 Integration with Crixly

ClipHub can be:
- **Option A**: A separate app that calls Crixly's API to install blueprints
- **Option B**: A built-in section of the Crixly UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Crixly UI and API.
...
- [ ] Install flow: blueprint → Crixly company
...
- [ ] "Export from Crixly" CLI command
