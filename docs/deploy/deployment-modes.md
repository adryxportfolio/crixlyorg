...
Crixly supports two runtime modes with different security profiles.
...
```sh
pnpm crixlyai allowed-hostname my-machine
```
...
When migrating from `local_trusted` to `authenticated`, Crixly emits a one-time claim URL at startup:
...
```sh
CRIXLY_DEPLOYMENT_MODE=authenticated pnpm crixlyai run
```
