...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Crixly.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.crixly.yaml`
...
- Crixly must not require extra top-level fields for skill validity
- Crixly-specific extensions must live under `metadata.crixly` or `metadata.sources`
...
For Crixly, the preferred fidelity extension is:
...
- issue/task Crixly-only metadata
...
- Crixly tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Crixly shape:
...
Additional rules for Crixly exporters:
...
- omit empty and default-valued Crixly fields when possible
...
- Crixly exporters should emit `.crixly.yaml` as a sidecar by default
...
Crixly can map this spec to its runtime model like this:
...
- Crixly extension:
  - `.crixly.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Crixly-specific fidelity

Inline Crixly-only metadata that must live inside a shared markdown file should use:

- `metadata.crixly`

That keeps the base format broader than Crixly.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Crixly.

## 20. Cutover

Crixly should cut over to this markdown-first package model as the primary portability format.

`crixly.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Crixly, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
