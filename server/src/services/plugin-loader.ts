 ...
 *    (`~/.crixly/plugins/`) and `node_modules` for packages matching
 *    the `crixly-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Crixly plugins.
 * Packages matching this pattern are considered Crixly plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "crixly-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: CrixlyPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.crixly/plugins/ local directory
  | "npm"               // npm packages matching crixly-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: CrixlyPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the crixly-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "crixly-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: CrixlyPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Crixly plugins installed as npm packages in the current
   * Node.js environment matching the "crixly-plugin-*" naming convention.
   *
   ...
   * the "crixlyPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Crixly plugin.
   * Throws if the package is a Crixly plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<CrixlyPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: CrixlyPluginManifestV1;
    newManifest: CrixlyPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Crixly plugin naming convention.
 * Accepts both the "crixly-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @crixlyai/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "crixlyPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const crixlyPlugin = pkgJson["crixlyPlugin"];
  if (
    crixlyPlugin !== null &&
    typeof crixlyPlugin === "object" &&
    !Array.isArray(crixlyPlugin)
  ) {
    const manifestRelPath = (crixlyPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: CrixlyPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumCrixlyVersion;
}
 ...
 *   packageName: "crixly-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: CrixlyPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as CrixlyPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Crixly plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<CrixlyPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Crixly plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasCrixlyPlugin = "crixlyPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasCrixlyPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<CrixlyPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: CrixlyPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @crixlyai/shared exports). Run those workers through
