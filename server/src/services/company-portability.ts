...
import type { Db } from "@crixlyai/db";
import type {
...
import {
  readCrixlySkillSyncPreference,
  writeCrixlySkillSyncPreference,
} from "@crixlyai/adapter-utils/server-utils";
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".crixly.yaml" || normalized === ".crixly.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const crixly = isPlainRecord(metadata?.crixly) ? metadata?.crixly as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.crixlySkillKey)
    ?? asString(crixly?.skillKey)
    ?? asString(crixly?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "crixly_bundled") {
    return `crixlyai/crixly/${slug}`;
  }
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .crixly.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .crixly.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .crixly.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .crixly.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Crixly will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .crixly.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .crixly.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .crixly.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .crixly.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .crixly.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .crixly.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .crixly.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .crixly.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .crixly.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[crixlyExtensionPath];
  if (selectedFiles.has(crixlyExtensionPath) && typeof extensionEntry === "string") {
    filtered[crixlyExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findCrixlyExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".crixly.yaml"] === "string") return ".crixly.yaml";
  if (typeof files[".crixly.yml"] === "string") return ".crixly.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.crixly.yaml") || entry.endsWith("/.crixly.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "crixly_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "crixlyai/crixly",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/crixlyai/crixly/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "crixly_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.crixlySkillKey = skill.key;
  metadata.crixly = {
    ...(isPlainRecord(metadata.crixly) ? metadata.crixly : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const crixlyExtensionPath = findCrixlyExtensionPath(normalizedFiles);
  const crixlyExtension = crixlyExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, crixlyExtensionPath) ?? "")
    : {};
  const crixlyCompany = isPlainRecord(crixlyExtension.company) ? crixlyExtension.company : {};
  const crixlySidebar = normalizePortableSidebarOrder(crixlyExtension.sidebar);
  const crixlyAgents = isPlainRecord(crixlyExtension.agents) ? crixlyExtension.agents : {};
  const crixlyProjects = isPlainRecord(crixlyExtension.projects) ? crixlyExtension.projects : {};
  const crixlyTasks = isPlainRecord(crixlyExtension.tasks) ? crixlyExtension.tasks : {};
  const crixlyRoutines = isPlainRecord(crixlyExtension.routines) ? crixlyExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(crixlyCompany.brandColor),
      logoPath: asString(crixlyCompany.logoPath) ?? asString(crixlyCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof crixlyCompany.requireBoardApprovalForNewAgents === "boolean"
          ? crixlyCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
    },
    sidebar: crixlySidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(crixlyAgents[slug]) ? crixlyAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(crixlyProjects[slug]) ? crixlyProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(crixlyTasks[slug]) ? crixlyTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(crixlyRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(crixlyRoutines[slug]) ? crixlyRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
  ...
  async function exportBundle(
    ...
    const crixlyAgentsOut: Record<string, Record<string, unknown>> = {};
    const crixlyProjectsOut: Record<string, Record<string, unknown>> = {};
    const crixlyTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const crixlyRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readCrixlySkillSyncPreference(
        ...
        crixlyAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      crixlyProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      crixlyTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      crixlyRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const crixlyExtensionPath = ".crixly.yaml";
    const crixlyAgents = Object.fromEntries(
      Object.entries(crixlyAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const crixlyProjects = Object.fromEntries(
      Object.entries(crixlyProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const crixlyTasks = Object.fromEntries(
      Object.entries(crixlyTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const crixlyRoutines = Object.fromEntries(
      Object.entries(crixlyRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[crixlyExtensionPath] = buildYamlFile(
      {
        schema: "crixly/v1",
        ...
        agents: Object.keys(crixlyAgents).length > 0 ? crixlyAgents : undefined,
        projects: Object.keys(crixlyProjects).length > 0 ? crixlyProjects : undefined,
        tasks: Object.keys(crixlyTasks).length > 0 ? crixlyTasks : undefined,
        routines: Object.keys(crixlyRoutines).length > 0 ? crixlyRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, crixlyExtensionPath);
  ...
  async function importBundle(
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const adapterConfigWithSkills = writeCrixlySkillSyncPreference(
          baseAdapterConfig,
          desiredSkills,
        );
