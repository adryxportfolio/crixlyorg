...
import type { AdapterRuntimeServiceReport } from "@crixlyai/adapter-utils";
import type { Db } from "@crixlyai/db";
import { workspaceRuntimeServices } from "@crixlyai/db";
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.CRIXLY_WORKSPACE_CWD = input.worktreePath;
  env.CRIXLY_WORKSPACE_PATH = input.worktreePath;
  env.CRIXLY_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.CRIXLY_WORKSPACE_BRANCH = input.branchName;
  env.CRIXLY_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.CRIXLY_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.CRIXLY_WORKSPACE_SOURCE = input.base.source;
  env.CRIXLY_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.CRIXLY_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.CRIXLY_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.CRIXLY_PROJECT_ID = input.base.projectId ?? "";
  env.CRIXLY_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.CRIXLY_AGENT_ID = input.agent.id;
  env.CRIXLY_AGENT_NAME = input.agent.name;
  env.CRIXLY_COMPANY_ID = input.agent.companyId;
  env.CRIXLY_ISSUE_ID = input.issue?.id ?? "";
  env.CRIXLY_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.CRIXLY_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.CRIXLY_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.CRIXLY_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.CRIXLY_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.CRIXLY_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.CRIXLY_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.CRIXLY_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.CRIXLY_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.CRIXLY_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.CRIXLY_PROJECT_ID = input.workspace.projectId ?? "";
  env.CRIXLY_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.CRIXLY_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".crixly", "worktrees");
