...
import type { CrixlyPluginManifestV1 } from "@crixlyai/shared";
...
export async function loadPluginModuleInSandbox(
  ...
  const loadModuleSync = (modulePath: string): Record<string, unknown> => {
    ...
    const sandboxArgs = {
      __crixly_exports: module.exports,
      __crixly_module: module,
      __crixly_require: requireInSandbox,
      __crixly_filename: realPath,
      __crixly_dirname: path.dirname(realPath),
    };
    // Temporarily inject args into the context, run, then remove to avoid pollution.
    Object.assign(context, sandboxArgs);
    const wrapped = `(function (exports, module, require, __filename, __dirname) {\n${code}\n})(__crixly_exports, __crixly_module, __crixly_require, __crixly_filename, __crixly_dirname)`;
