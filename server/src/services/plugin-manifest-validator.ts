 ...
 */
import { pluginManifestV1Schema } from "@crixlyai/shared";
import type { CrixlyPluginManifestV1 } from "@crixlyai/shared";
import { PLUGIN_API_VERSION } from "@crixlyai/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): CrixlyPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as CrixlyPluginManifestV1,
        };
      }
