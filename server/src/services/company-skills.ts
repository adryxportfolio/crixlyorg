...
import { companySkills } from "@crixlyai/db";
import { readCrixlySkillSyncPreference, writeCrixlySkillSyncPreference } from "@crixlyai/adapter-utils/server-utils";
import type { CrixlySkillEntry } from "@crixlyai/adapter-utils/server-utils";
import type {
  CompanySkill,
...
import { normalizeAgentUrlKey } from "@crixlyai/shared";
import { findServerAdapter } from "../adapters/index.js";
import { resolveCrixlyInstanceRoot } from "../home-paths.js";
...
function buildSkillRuntimeName(key: string, slug: string) {
  if (key.startsWith("crixlyai/crixly/")) return slug;
...
function resolveManagedSkillsRoot(companyId: string) {
  return path.resolve(resolveCrixlyInstanceRoot(), "skills", companyId);
}
...
function deriveSkillSourceInfo(skill: CompanySkill): {
  ...
  if (metadata.sourceKind === "crixly_bundled") {
    return {
      editable: false,
      editableReason: "Bundled Crixly skills are read-only.",
      sourceLabel: "Crixly bundled",
      sourceBadge: "crixly",
      sourcePath: null,
    };
  }
...
export function companySkillService(db: Db) {
        ...
        if (!adapter?.listSkills) {
          ...
          try {
            ...
            const snapshot = await adapter.listSkills({
              ...
              config: {
                ...runtimeConfig,
                crixlyRuntimeSkills: runtimeSkillEntries,
              },
            });
  ...
  async function listRuntimeSkillEntries(
    ...
    for (const skill of skills) {
      ...
      const required = sourceKind === "crixly_bundled";
      out.push({
        key: skill.key,
        ...
        requiredReason: required
          ? "Bundled Crixly skills are always available for local adapters."
          : null,
      });
    }
  ...
  async function upsertImportedSkills(companyId: string, imported: ImportedSkill[]): Promise<CompanySkill[]> {
    ...
    for (const skill of imported) {
      ...
      if (
        existing
        && existingMeta.sourceKind === "crixly_bundled"
        && incomingKind === "github"
        && incomingOwner === "crixlyai"
        && incomingRepo === "crixly"
      ) {
        out.push(existing);
        continue;
    ...
    for (const agent of agentRows) {
      ...
      if (referencesSkill) {
        ...
        await agents.update(agent.id, {
          adapterConfig: writeCrixlySkillSyncPreference(config, filtered),
        });
      }
    }
