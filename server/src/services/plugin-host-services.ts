...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@crixlyai/db";
 ...
 *
 * This implementation delegates to the core Crixly domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@crixlyai/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
