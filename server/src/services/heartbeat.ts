...
import type { BillingType } from "@crixlyai/shared";
import {
  agents,
  agentRuntimeState,
  agentTaskSessions,
...
const DEFERRED_WAKE_CONTEXT_KEY = "_crixlyWakeContext";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
const startLocksByAgent = new Map<string, Promise<void>>();
const REPO_ONLY_CWD_SENTINEL = "/__crixly_repo_only__";
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    context.crixlyWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.crixlyRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.crixlyRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.crixlySessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.crixlySessionRotationReason = sessionCompaction.reason;
      context.crixlyPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.crixlySessionHandoffMarkdown;
      delete context.crixlySessionRotationReason;
      delete context.crixlyPreviousSessionId;
    }
    ...
    try {
      ...
      if (runtimeServices.length > 0) {
        context.crixlyRuntimeServices = runtimeServices;
        context.crixlyRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[crixly] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected CRIXLY_API_KEY",
        );
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.crixlyRuntimeServices = combinedRuntimeServices;
        context.crixlyRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[crixly] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
