...
import { ensureCursorSkillsInjected } from "@crixlyai/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing Crixly skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("crixly-cursor-skills-src-");
    const skillsHome = await makeTempDir("crixly-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "crixly");
    await createSkillDir(skillsDir, "crixly-create-agent");
    ...
    const injectedA = path.join(skillsHome, "crixly");
    const injectedB = path.join(skillsHome, "crixly-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "crixly")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "crixly-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "crixly"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "crixly-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("crixly-cursor-preserve-src-");
    const skillsHome = await makeTempDir("crixly-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "crixly");
    ...
    expect((await fs.lstat(path.join(skillsHome, "crixly-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("crixly-cursor-fail-src-");
    const skillsHome = await makeTempDir("crixly-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
