...
import { execute, testEnvironment } from "@crixlyai/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@crixlyai/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("crixly:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("CRIXLY_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("CRIXLY_TASK_ID=task-123");
