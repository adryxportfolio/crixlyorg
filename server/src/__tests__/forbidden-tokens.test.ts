...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "crixly", LOGNAME: "crixly", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "crixly" }),
      },
    );

    expect(tokens).toEqual(["crixly", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "crixly" }, {
        userInfo: () => ({ username: "crixly" }),
      });

      expect(tokens).toEqual(["crixly", "custom-token"]);
    } finally {
