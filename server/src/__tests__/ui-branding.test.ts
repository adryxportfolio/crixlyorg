...
<head>
    <!-- CRIXLY_RUNTIME_BRANDING_START -->
    <!-- CRIXLY_RUNTIME_BRANDING_END -->
    <!-- CRIXLY_FAVICON_START -->
...
describe("ui branding", () => {
  it("detects worktree mode from CRIXLY_IN_WORKTREE", () => {
    expect(isWorktreeUiBrandingEnabled({ CRIXLY_IN_WORKTREE: "true" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ CRIXLY_IN_WORKTREE: "1" })).toBe(true);
    expect(isWorktreeUiBrandingEnabled({ CRIXLY_IN_WORKTREE: "false" })).toBe(false);
  });

  it("resolves name, color, and text color for worktree branding", () => {
    const branding = getWorktreeUiBranding({
      CRIXLY_IN_WORKTREE: "true",
      CRIXLY_WORKTREE_NAME: "crixly-pr-432",
      CRIXLY_WORKTREE_COLOR: "#4f86f7",
    });

    expect(branding.enabled).toBe(true);
    expect(branding.name).toBe("crixly-pr-432");
  ...
  it("renders runtime branding metadata for the ui", () => {
    ...
    expect(meta).toContain('name="crixly-worktree-name"');
    expect(meta).toContain('content="crixly-pr-432"');
    expect(meta).toContain('name="crixly-worktree-color"');
  });
  ...
  it("rewrites the favicon and runtime branding blocks for worktree instances only", () => {
    ...
    expect(branded).toContain('name="crixly-worktree-name"');
    ...
    expect(defaultHtml).not.toContain('name="crixly-worktree-name"');
  });
});
