...
describe("agent local JWT", () => {
  const secretEnv = "CRIXLY_AGENT_JWT_SECRET";
  const ttlEnv = "CRIXLY_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "CRIXLY_AGENT_JWT_ISSUER";
  const audienceEnv = "CRIXLY_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "crixly";
    process.env[audienceEnv] = "crixly-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
