...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Crixly OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Crixly base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("crixlyApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.crixlyApiUrl");
    expect(text).toContain("~/.openclaw/workspace/crixly-claimed-api-key.json");
    expect(text).toContain("CRIXLY_API_KEY");
