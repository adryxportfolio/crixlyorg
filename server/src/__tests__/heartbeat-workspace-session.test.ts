...
import type { agents } from "@crixlyai/db";
import { sessionCodec as codexSessionCodec } from "@crixlyai/adapter-codex-local/server";
...
describe("formatRuntimeWorkspaceWarningLog", () => {
  it("emits informational workspace warnings on stdout", () => {
    expect(formatRuntimeWorkspaceWarningLog("Using fallback workspace")).toEqual({
      stream: "stdout",
      chunk: "[crixly] Using fallback workspace\n",
    });
  });
});
...
describe("parseSessionCompactionPolicy", () => {
  it("disables Crixly-managed rotation by default for codex and claude local", () => {
