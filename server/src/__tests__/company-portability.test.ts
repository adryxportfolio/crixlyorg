...
import type { CompanyPortabilityFileEntry } from "@crixlyai/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "crixly",
        name: "crixly",
        description: "Crixly coordination skill",
        markdown: "---\nname: crixly\ndescription: Crixly coordination skill\n---\n\n# Crixly\n",
        sourceType: "github",
        sourceLocator: "https://github.com/crixlyai/crixly/tree/master/skills/crixly",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/crixlyai/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      owner: "crixlyai",
      repo: "companies",
      ref: "feature/demo",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/crixlyai/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      owner: "crixlyai",
      repo: "companies",
  ...
  it("exports referenced skills as stubs by default with sanitized Crixly extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Crixly"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${crixlyKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/crixlyai/crixly/crixly/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/crixlyai/crixly/crixly/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/crixlyai/crixly/crixly/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".crixly.yaml"]);
    expect(extension).toContain('schema: "crixly/v1"');
    ...
    expect(extension).not.toContain("crixlySkillSync");
  ...
  it("exports default sidebar order into the Crixly extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".crixly.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/crixlyai/crixly/crixly/SKILL.md"])).toContain("# Crixly");
    expect(asTextFile(exported.files["skills/crixlyai/crixly/crixly/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/crixlyai/crixly/crixly/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/crixlyai/crixly/crixly/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/crixlyai/crixly/crixly/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .crixly.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".crixly.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-crixly",
        companyId: "company-1",
        key: "crixlyai/crixly/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/crixlyai/crixly/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "crixly_bundled",
          owner: "crixlyai",
          repo: "crixly",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/crixlyai/crixly/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/crixlyai/crixly/release-changelog/SKILL.md"])).toContain("crixlyai/crixly/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/crixly",
            repoUrl: "https://github.com/crixlyai/crixly.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .crixly-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/crixlyai/crixly.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/crixly");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Crixly",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "crixly-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/crixlyai/crixly.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".crixly.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/crixlyai/crixly.git"');
    expect(extension).toContain('projectWorkspaceKey: "crixly"');
  ...
  it("reads env inputs back from .crixly.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Crixly routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "crixly-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Crixly" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Crixly"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .crixly.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Crixly");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".crixly.yaml"] = `${exported.files[".crixly.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
