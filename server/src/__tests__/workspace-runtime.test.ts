...
import type { WorkspaceOperation } from "@crixlyai/shared";
...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crixly-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "crixly@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "Crixly Test"]);
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".crixly", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$CRIXLY_WORKSPACE_BRANCH\" > .crixly-provision-branch",
        "printf '%s\\n' \"$CRIXLY_WORKSPACE_BASE_CWD\" > .crixly-provision-base",
        "printf '%s\\n' \"$CRIXLY_WORKSPACE_CREATED\" > .crixly-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".crixly-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".crixly-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".crixly-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".crixly-provision-created"), "utf8")).resolves.toBe("false\n");
  });
...
describe("ensureRuntimeServicesForRun", () => {
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crixly-runtime-workspace-"));
  ...
  it("does not leak parent Crixly instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crixly-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "crixlyConfig: process.env.CRIXLY_CONFIG ?? null,",
          "crixlyHome: process.env.CRIXLY_HOME ?? null,",
          "crixlyInstanceId: process.env.CRIXLY_INSTANCE_ID ?? null,",
    ...
    process.env.CRIXLY_CONFIG = "/tmp/base-crixly-config.json";
    process.env.CRIXLY_HOME = "/tmp/base-crixly-home";
    process.env.CRIXLY_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/crixly";
    ...
    expect(captured.crixlyConfig).toBeNull();
    expect(captured.crixlyHome).toBeNull();
    expect(captured.crixlyInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "crixly-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "crixly-runtime-sibling-"));
