...
describe("agent instructions service", () => {
  const originalCrixlyHome = process.env.CRIXLY_HOME;
  const originalCrixlyInstanceId = process.env.CRIXLY_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalCrixlyHome === undefined) delete process.env.CRIXLY_HOME;
    else process.env.CRIXLY_HOME = originalCrixlyHome;
    if (originalCrixlyInstanceId === undefined) delete process.env.CRIXLY_INSTANCE_ID;
    else process.env.CRIXLY_INSTANCE_ID = originalCrixlyInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const crixlyHome = await makeTempDir("crixly-agent-instructions-home-");
    const externalRoot = await makeTempDir("crixly-agent-instructions-external-");
    cleanupDirs.add(crixlyHome);
    cleanupDirs.add(externalRoot);
    process.env.CRIXLY_HOME = crixlyHome;
    process.env.CRIXLY_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("crixly-agent-instructions-new-external-");
    cleanupDirs.add(crixlyHome);
    cleanupDirs.add(externalRoot);
    process.env.CRIXLY_HOME = crixlyHome;
    process.env.CRIXLY_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("crixly-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const crixlyHome = await makeTempDir("crixly-agent-instructions-recover-");
    cleanupDirs.add(crixlyHome);
    process.env.CRIXLY_HOME = crixlyHome;
    process.env.CRIXLY_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const crixlyHome = await makeTempDir("crixly-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("crixly-agent-instructions-stale-root-");
    cleanupDirs.add(crixlyHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const crixlyHome = await makeTempDir("crixly-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("crixly-agent-instructions-heal-write-stale-");
    cleanupDirs.add(crixlyHome);
    cleanupDirs.add(staleRoot);
    process.env.CRIXLY_HOME = crixlyHome;
    process.env.CRIXLY_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const crixlyHome = await makeTempDir("crixly-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("crixly-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(crixlyHome);
    cleanupDirs.add(staleRoot);
    process.env.CRIXLY_HOME = crixlyHome;
    process.env.CRIXLY_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const crixlyHome = await makeTempDir("crixly-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("crixly-agent-instructions-partial-root-");
    cleanupDirs.add(crixlyHome);
    cleanupDirs.add(staleRoot);
    process.env.CRIXLY_HOME = crixlyHome;
    process.env.CRIXLY_INSTANCE_ID = "test-instance";
