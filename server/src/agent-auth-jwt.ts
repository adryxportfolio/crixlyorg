...
function jwtConfig() {
  const secret = process.env.CRIXLY_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.CRIXLY_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.CRIXLY_AGENT_JWT_ISSUER ?? "crixly",
    audience: process.env.CRIXLY_AGENT_JWT_AUDIENCE ?? "crixly-api",
  };
}
