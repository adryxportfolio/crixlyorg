...
import type { DeploymentExposure, DeploymentMode } from "@crixlyai/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "crixly" &&
    normalized !== "crixly-create-agent" &&
    normalized !== "crixly-create-plugin" &&
...
/** Resolve the Crixly repo skills directory (built-in / managed skills). */
function resolveCrixlySkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const crixlySkillsDir = resolveCrixlySkillsDir();

  // Build set of Crixly-managed skill names
  const crixlySkillNames = new Set<string>();
  if (crixlySkillsDir) {
    try {
      for (const entry of fs.readdirSync(crixlySkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) crixlySkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isCrixlyManaged: crixlySkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.crixlyApiUrl)) {
    const legacyCrixlyApiUrl = nonEmptyTrimmedString(input.crixlyApiUrl);
    if (legacyCrixlyApiUrl) merged.crixlyApiUrl = legacyCrixlyApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    crixlyApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.crixlyApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawCrixlyApiUrl =
    typeof defaults.crixlyApiUrl === "string"
      ? defaults.crixlyApiUrl.trim()
      : "";
  if (rawCrixlyApiUrl) {
    try {
      const parsedCrixlyApiUrl = new URL(rawCrixlyApiUrl);
      if (
        parsedCrixlyApiUrl.protocol !== "http:" &&
        parsedCrixlyApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_crixly_api_url_protocol",
          level: "warn",
          message: `crixlyApiUrl must use http:// or https:// (got ${parsedCrixlyApiUrl.protocol}).`
        });
      } else {
        normalized.crixlyApiUrl = parsedCrixlyApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_crixly_api_url_configured",
          level: "info",
          message: `crixlyApiUrl set to ${parsedCrixlyApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_crixly_api_url_invalid",
        level: "warn",
        message: `Invalid crixlyApiUrl: ${rawCrixlyApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Crixly host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Crixly is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm crixlyai allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/crixly";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/crixly-claimed-api-key.json and load CRIXLY_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: crixlyApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Crixly hostname is reachable and allowed via `pnpm crixlyai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Crixly API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/crixly/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Crixly OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          crixlyApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Crixly generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "crixlyApiUrl": "https://crixly-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Crixly before key claim is allowed.
    ...
    - ~/.openclaw/workspace/crixly-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/crixly-claimed-api-key.json

    And set the CRIXLY_API_KEY and CRIXLY_API_URL in your environment variables as specified here:
    ...
    Then set CRIXLY_API_KEY and CRIXLY_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Crixly skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your CRIXLY_API_URL to the top of your skill and note the path to your CRIXLY_API_URL
    ...
    ${
      ...
      "Ensure Crixly is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Crixly base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.crixlyApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm crixlyai allowed-hostname <host>
      - then restart Crixly and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@crixly.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "crixly", path: "/api/skills/crixly" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "crixly-create-agent",
          path: "/api/skills/crixly-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              crixlyApiUrl: req.body.crixlyApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.crixlyApiUrl &&
          !persistedDefaults.crixlyApiUrl
        ) {
          missingPersistedFields.push("crixlyApiUrl");
        }
