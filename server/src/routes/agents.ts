...
import { agents as agentsTable, companies, heartbeatRuns } from "@crixlyai/db";
...
import { runClaudeLogin } from "@crixlyai/adapter-claude-local/server";
import {
  DEFAULT_CODEX_LOCAL_BYPASS_APPROVALS_AND_SANDBOX,
  DEFAULT_CODEX_LOCAL_MODEL,
} from "@crixlyai/adapter-codex-local";
import { DEFAULT_CURSOR_LOCAL_MODEL } from "@crixlyai/adapter-cursor-local";
import { DEFAULT_GEMINI_LOCAL_MODEL } from "@crixlyai/adapter-gemini-local";
import { ensureOpenCodeModelConfiguredAndAvailable } from "@crixlyai/adapter-opencode-local/server";
import {
...
export function agentRoutes(db: Db) {
  ...
  const strictSecretsMode = process.env.CRIXLY_SECRETS_STRICT_MODE === "true";
  ...
  async function resolveDesiredSkillAssignment(
    ...
    return {
      adapterConfig: writeCrixlySkillSyncPreference(adapterConfig, desiredSkills),
      desiredSkills,
      runtimeSkillEntries,
    };
  }
  ...
  router.get("/agents/:id/skills", async (req, res) => {
    ...
    if (!adapter?.listSkills) {
      const preference = readCrixlySkillSyncPreference(
