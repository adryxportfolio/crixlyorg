const FAVICON_BLOCK_START = "<!-- CRIXLY_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- CRIXLY_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- CRIXLY_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- CRIXLY_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.CRIXLY_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.CRIXLY_WORKTREE_NAME) ?? nonEmpty(env.CRIXLY_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.CRIXLY_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="crixly-worktree-enabled" content="true" />',
    `<meta name="crixly-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="crixly-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="crixly-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
