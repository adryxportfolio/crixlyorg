...
import { resolveCrixlyConfigPath, resolveCrixlyEnvPath } from "./paths.js";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = process.env.CRIXLY_AGENT_JWT_SECRET?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.CRIXLY_AGENT_JWT_SECRET === "string" ? parsed.CRIXLY_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm crixlyai onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveCrixlyEnvPath();
