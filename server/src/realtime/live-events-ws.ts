...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@crixlyai/db";
import type { DeploymentMode } from "@crixlyai/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  crixlyUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).crixlyUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.crixlyUpgradeContext = context;
