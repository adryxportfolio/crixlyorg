---
name: crixly-create-plugin
description: >
  Create new Crixly plugins with the current alpha SDK/runtime. Use when
...
# Create a Crixly Plugin

Use this skill when the task is to create, scaffold, or document a Crixly plugin.
...
node packages/plugins/create-crixly-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Crixly repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.crixly-sdk/`:

```bash
pnpm --filter @crixlyai/create-crixly-plugin build
node packages/plugins/create-crixly-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Crixly from an absolute local path during development
